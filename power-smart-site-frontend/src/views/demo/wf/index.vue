  ...
  <div>
    <div class="c-container">
      <flexbox v-if="!$slots.icon" class="title"><img   :onerror='`this.src="${require("@/assets/noimgsrc.png")}"`'   
        :src="titleIcon"
        class="title-icon">DEMO（审批）管理
      </flexbox>
      <slot v-else name="icon"/>
      <el-input
        placeholder="请输入编号/内容"
      ...
      <flexbox
        ...
        <flexbox class="selection-items-box">
          <el-button
            ...
            :key="index"
            type="primary"
            @click.native="handleTypeDrop(item.type)">{{ item.name }}</el-button>
        </flexbox>
      </flexbox>
    </div>
    <div
...
<script>
  ...
  import {getList,delTrueByIds } from "./api";
  ...
  export default {
    ...
    data() {
      return {
        ...
        fieldList: [//表头
          ...
          {
            prop: "examineRecordId",
            label: "ExamineRecordId",
            width: "180"
          },
        ],
        sortData: {}, // 字段排序
    ...
    methods: {
      ...
      updateTableHeight() {
        ...
        var removeHeight = this.selectionList.length > 0 ? 230 : 180
