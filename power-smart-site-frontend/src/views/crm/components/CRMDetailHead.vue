...
import { crmExpenseDeleteAPI,crmExpenseExamineDeleteAPI } from '@/api/crm/expense'
...
export default {
  ...
  computed: {
    ...
    name() {
      ...
      } else if (this.crmType === 'expense') {
        return this.detail.expenseNum
      } else if (this.crmType === 'expense-examines') {
    ...
    showTransfer() {
      if (
        this.crmType === 'expense' ||
        this.crmType === 'expense-examines' ||
    ...
    showEdit() {
      ...
      if(this.crmType === 'expense-examines'){
        return this.crm.expense.update
      }
  ...
  methods: {
    ...
    handleTypeClick(type) {
      if (type == 'edit') {
        ...
        //window.open('http://127.0.0.1:8449/'+routeData.href, '_blank')
        this.$router.push(`/crm/contract/print?module=${this.crmType}&id=${id}`);
      }
    },
    confirmHandle(type) {
      if (type === 'lock' || type === 'unlock') {
        ...
        const request = {
          ...
          expense: crmExpenseDeleteAPI,
          'expense-examines': crmExpenseExamineDeleteAPI,
        }[this.crmType]
        const params = {}
        if (this.isSeas) {
    ...
    whetherTypeShowByPermision(type) {
      if (type == 'transfer') {
        ...
        if(this.crmType === 'expense-examines'){
          return this.crm.expense.delete
        }
