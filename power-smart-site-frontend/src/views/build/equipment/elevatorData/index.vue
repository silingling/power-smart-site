<template>
	<div>
		...
		<div class="table-head-container">
			...
			<el-table v-loading="loading" id="crm-table" @sort-change="sortChange" :data="list" :height="tableHeight"
				...
				<el-table-column v-for="(item, index) in fieldList" :key="index" :prop="item.prop" :label="item.label"
					:width="item.width" :formatter="fieldFormatter" sortable="custom" align="center" show-overflow-tooltip>
  ...
  import { sortColumn } from '@/utils/index';
	...
	export default {
		...
		mounted() {
			...
			console.log("aaaa", this.fieldList)
			// 初始化时间，默认为今天
		},
		watch: {
			//监听页面切换
		...
		methods: {
			...
			getList() {
				...
				selectElevatorPage(this.queryParams)
					.then(res => {
