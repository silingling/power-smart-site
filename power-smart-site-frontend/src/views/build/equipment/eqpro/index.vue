  ...
  <div>
    ...
    <el-form v-show="true" ref="queryForm" :model="queryParams" :inline="true" label-width="80px"
      ...
      <el-form-item label="工程类型" prop="name">
        <el-select v-model="queryParams.type" clearable placeholder="请选择工程类型">
...
<script>
  ...
  import {getList, delTrueByIds, changeStatus} from "../api/eqpro"
    ...
    data() {
      return {
        ...
        fieldList: [//表头
          ...
          {
            prop: "planeCoordinateX",
            label: "相对位置X",
            align: 'center'
          },
          {
            prop: "planeCoordinateY",
            label: "相对位置Y",
            align: 'center'
          },
          {
    ...
    methods: {
      ...
      fieldFormatter(row, column, cellValue) {
        if (column.property === 'status') {
          var statusName = '';
          this.statusDict.forEach(status => {
            if(status.dictValue == row.status){
              statusName = status.dictLabel;
            }
          })
          return statusName;
        }
        if (column.property === 'type') {
          return row.type === '10' ? '基坑工程' : '高支模工程'
        }
...
//    },
      ...
      updateStatus(id, status, rmk) {
        this.$confirm("确定" + rmk + "此配置吗？", '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        })
