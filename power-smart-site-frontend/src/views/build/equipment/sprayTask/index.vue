  ...
  <div>
    ...
    <el-form v-show="true" ref="queryForm" :model="queryParams" :inline="true" label-width="80px"
	  ...
	  <el-form-item label="启用状态" prop="isEnable">
	    <el-select v-model="queryParams.isEnable" placeholder="请选择是否启用">
	      <el-option
    ...
    <div
      ...
      <el-table
        ...
        <el-table-column
          ...
          align="center"
          width="55"/>
        <el-table-column label="序号" type="index" align="center" show-overflow-tooltip width="70" fixed />
        <el-table-column
...
<script>
  ...
  import {getList, delTrueByIds} from "../api/sprayTask"
  ...
  export default {
    ...
    methods: {
    	sortChange(column) {
    		if(column.prop=='duration'){
    			column.prop='duration+0'
    		}
    		if(column.prop=='cycle'){
    			column.prop='cycle+0'
    		}
      ...
      fieldFormatter(row, column, cellValue) {
		  if (column.property === 'cycleUnit') {
		  	return this.selectDictLabel(this.dictData.cycleUnitDict, row.cycleUnit)
...
//    },
	  ...
	  // 初始化字典数据
	  initDicts() {
	  	this.getDicts('b_equipment_spray_cycle_unit').then(response => {
	  	  this.dictData.cycleUnitDict = response.data
	  	})
	  },
    }
  }
