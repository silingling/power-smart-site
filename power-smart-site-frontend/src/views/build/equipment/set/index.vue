<template>
  <div class="main">
    ...
    <div class="main-body">
      <el-form class="searchFrom" v-show="showSearch" ref="queryForm" :model="queryParams" :inline="true" label-width="68px" style="padding: 20px 20px 0px; background-color: white; border-top: 0px solid rgb(230, 230, 230);">
        ...
        <el-form-item label="计划类型">
          <el-select v-model="queryParams.planType" clearable placeholder="请选择计划类型"
                     @keyup.enter.native="handleQuery">
            <el-option
      ...
      <el-table v-loading="loading" :data="tableList"
                :height="tableHeight" highlight-current-row @sort-change="sortChange">
        <el-table-column label="序号" type="index" show-overflow-tooltip width="70"></el-table-column>
        <el-table-column label="执行编号" prop="setCode" show-overflow-tooltip sortable="custom"/>
        <el-table-column label="执行部门" prop="deptName" show-overflow-tooltip sortable="custom"/>
        <el-table-column label="计划名称" prop="planName" show-overflow-tooltip sortable="custom"/>
        <el-table-column label="计划类型" prop="planType" :formatter="planTypeFormat" show-overflow-tooltip sortable="custom"/>
        <el-table-column label="执行日期" prop="setDate" show-overflow-tooltip sortable="custom">
          <template slot-scope="scope">
            <span>{{ dateFormat(scope.row.setDate) }}</span>
          </template>
        </el-table-column>
        <el-table-column label="执行周期" prop="cycle" width="150px" show-overflow-tooltip sortable="custom">
        </el-table-column>
        <el-table-column label="周期单位" prop="unit" :formatter="unitFormat" show-overflow-tooltip sortable="custom"/>
      ...
      <!-- 修改日期 -->
      <el-dialog title="修改日期" :visible.sync="openDate" width="400px" append-to-body>
        <el-form ref="formDate" :model="formDate" :rules="rulesDate" label-width="80px" class="new-dialog-form">
          <el-form-item label="执行日期" prop="setDate">
            <el-date-picker
              v-model="formDate.setDate"
              type="date"
              ...
              <el-row>
                <el-col :span="24">
                  <el-form-item label="执行人" prop="setBy">
                    <el-input v-model="form.setBy" placeholder="请输入执行人"></el-input>
                  </el-form-item>
                </el-col>
              </el-row>
              <el-row>
                <el-col :span="24">
                  <el-form-item label="执行情况说明" prop="setContent">
                    <el-input type="textarea" :rows="2" v-model="form.setContent" placeholder="请输入执行情况说明"></el-input>
                  </el-form-item>
                </el-col>
              </el-row>
            </el-form>
