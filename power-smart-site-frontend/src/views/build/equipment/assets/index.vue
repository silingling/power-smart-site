<template>
  <div class="employee-dep-management">
      ...
      <div class="system-view-table flex-index" v-loading="formLoading">
        ...
        <div class="flex-box">
          <el-table
            ...
            <el-table-column
              prop="assetName"
              label="设备名称" width="180" show-overflow-tooltip sortable="custom">
            ...
            <el-table-column
              prop="deptName"
              label="负责部门"
              width="140" show-overflow-tooltip sortable="custom">
            </el-table-column>
            <el-table-column
              prop="typeName"
              label="设备类型"
            ...
            <el-table-column
              ...
              label="状态"
              width="80"
              :formatter="offlineStatusFormatter"
              sortable="custom"
            >
            </el-table-column>
            <el-table-column
              ...
              <template slot-scope="scope">
                <el-button @click="jionConfig(scope.row)" type="text" size="small" icon="el-icon-edit">对接配置
                </el-button>
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="项目名称" prop="projectId">
              <el-select style="width: 100%;" v-model="form.projectId" placeholder="请选择项目名称">
                <el-option
                  v-for="item in projectInfoList"
                  :key="item.id"
                  :label="item.name"
        ...
        <el-tab-pane label="设备信息" name="asset">
          <el-form
            ...
            <el-row>
              ...
              <el-col :span="12">
                <el-form-item label="项目名称" prop="projectId">
                  <el-select disabled style="width: 100%;" v-model="form.projectId" placeholder="请选择项目名称">
                    <el-option
                      v-for="item in projectInfoList"
                      :key="item.id"
                      :label="item.name"
            ...
            <div v-if="showExpend">
              ...
              <component
                key="expendDetail"
                ref="expendFormDetail"
                :is="componentNameDetail"
              />
            </div>
            <el-row style="border-bottom: 1px solid #e6e6e6;padding-bottom: 10px;margin-bottom: 20px;">
              <span>其他信息</span>
    ...
    <!-- 对接配置 -->
    <el-dialog
      v-loading="jionformLoading"
      :visible.sync="jionformDialog"
      title="对接配置"
      :close-on-click-modal="false"
      :modal-append-to-body="true"
      :append-to-body="true"
    ...
    >
      ...
      >
        <el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item label="对接平台" prop="platform">
                <el-select v-model="jionform.platform" placeholder="请选择对接平台">
                  <el-option
                    v-for="item in jionformDict"
                    :key="item.id"
                    :label="item.name"
                    :value="item.id">
                  </el-option>
          ...
          <el-col :span="24">
            <el-form-item label="平台设备编码" prop="platformEqCode">
              <el-input v-model="jionform.platformEqCode" placeholder="请输入平台设备编码"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="数据解析器" prop="dataResolver">
              <el-input v-model="jionform.dataResolver" placeholder=""></el-input>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      jionformDict: [],
      jionform: {},
      jionformLoading: false,
      jionformDialog: false,
      //扩展配置
      showExpend: false,
      eqID: -1,
      action: 'insert',
  ...
  mounted() {
    ...
    getList({status: '1'})
      .then(res => {
        this.jionformDict = res.data == null ? [] : res.data;
      })
      .catch(() => {
      })
  },
  methods: {
    ...
    getTypeById(id) {
      typeGet(id)
        .then(data => {
          if (data.code == 0) {
            console.log(data)
            if (data.data.ifExpInfo == '1') {
              this.showExpend = true;
              this.componentName = this.loadView(data.data.expInfoCom);
              this.componentNameDetail = this.loadView(data.data.expInfoCom);
            } else {
    ...
    jionConfig(data) {
      this.jionformDialog = true;
      assetGet(data.assetId)
        .then(response => {
          this.jionform = response.data || []
        })
        .catch(() => {
        })
    },
    jionSave() {
      this.$refs['jionform'].validate((valid) => {
        if (valid) {
          assetSave(this.jionform)
            .then(data => {
              if (data.code == 0) {
                this.$message({
                  message: '保存成功',
    ...
    add() {
      ...
      //扩展信息
      if (this.showExpend) {
        this.$refs['expendForm'].expendInit();
      }
      ...
      assetGet(data.assetId)
        .then(response => {
          if (response.data.planeCoordinate) {
            var strs = response.data.planeCoordinate.split(",");
            response.data.planeCoordinateX = Number(strs[0]);
            response.data.planeCoordinateY = Number(strs[1]);
          }
          ...
          //扩展信息
          if (this.showExpend) {
            this.$refs['expendForm'].expendInit(data.assetId);
          }
        })
        .catch(() => {
        })
    ...
    save() {
      this.$refs['equform'].validate((valid) => {
        if (valid) {
          //是否核对扩展
          if (this.showExpend) {
            var flag = this.$refs['expendForm'].expendCheck();
            if (!flag) {
              return;
            }
          }
          ...
          assetSave(this.form)
            .then(data => {
              if (data.code == 0) {
                ...
                //保存扩展信息
                if (that.showExpend) {
                  that.$refs['expendForm'].expendSave(data.data);
                }
                that.$message({
                  message: '保存成功',
                  type: 'success'
        ...
        assetDelete(data)
          .then(res => {
            if (res.code == 0) {
              //删除扩展信息
              if (this.showExpend) {
                this.$refs['expendForm'].expendDelete(data.assetId);
              }
              this.$message({
                message: '删除成功',
                type: 'success'
      ...
      assetGet(data.assetId)
        .then(response => {
          if (this.form.planeCoordinate) {
            let strs = response.data.planeCoordinate.split(",");
            response.data.planeCoordinateX = Number(strs[0]);
            response.data.planeCoordinateY = Number(strs[1]);
          }
          this.form = response.data || []
          ...
          //扩展信息
          if (this.showExpend) {
            this.$refs['expendFormDetail'].expendInit(data.assetId);
          }
        })
        .catch(() => {
        })
    ...
    offlineStatusFormatter(row, column, cellValue) {
      if (cellValue == '1') {
        return '在线'
      } else {
        return '离线'
    ...
    cancelClick(data) {
      ...
      // 更新设备状态
      assetSave(params).then(response => {
        if (response.code === 0) {
          this.$message({
            message: '状态更新成功',
            type: 'success'
