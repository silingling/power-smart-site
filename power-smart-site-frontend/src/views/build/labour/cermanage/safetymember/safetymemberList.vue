      ...
      <el-table v-loading="loading" id="crm-table" :data="list" :height="tableHeight" class="n-table--border" use-virtual stripe border highlight-current-row style="width: 100%" @row-click="handleRowClick" @selection-change="handleSelectionChange" @sort-change="sortChange">
        ...
        <el-table-column v-for="(item, index) in fieldList" :key="index" :fixed="index==0" :prop="item.prop" :label="item.label" :width="item.width" :formatter="fieldFormatter" sortable="custom" align="center" show-overflow-tooltip>
          <template v-if="item.children">
            <el-table-column v-for="(i2,index2) in item.children" :key="index2" sortable="custom" :prop="i2.prop" :label="i2.label" align="center" :width="i2.width" :formatter="fieldFormatter" show-overflow-tooltip>
            </el-table-column>
          </template>
        </el-table-column>
...
import { getList, delByIds, selectFBSInfo, selectWorkTypeInfo } from "./safetymember";
...
/*              width: "210",*/
  ...
  methods: {
    ...
    fieldFormatter (row, column, cellValue) {
      ...
      if (column.property == "certificateName") {
        return this.selectDIYLabel(this.certificateNameOptions, row.certificateName, 'dictValue', 'dictLabel')
      }
