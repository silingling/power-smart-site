		...
		<div class="main-body">
			<el-form v-show="showSearch" ref="queryForm" :model="queryParams" :scroll-wheel-zoom="true" :inline="true" label-width="80px" style="padding-top: 20px;padding-left:10px;">
				<el-form-item label="施工单位" prop="companyName">
					<el-input v-model="queryParams.companyName" placeholder="请输入施工单位名称" clearable size="small" @keyup.enter.native="handleQuery" />
				</el-form-item>
				...
				<el-form-item label="项目状态" prop="projectState">
					<el-select v-model="queryParams.projectState" placeholder="请选择" clearable size="small" style="width: 200px">
						<el-option v-for="dict in projectStateOptions" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
					</el-select>
				</el-form-item>
				...
				<el-form-item style="float: right;">
					...
					<el-button type="primary" @click="syncAccount">同步账号</el-button>
				</el-form-item>
			</el-form>
			<el-table v-loading="loading" :height="tableHeight" v-if="isList" :data="typeList" @sort-change="sortChange">
				<el-table-column label="序号" type="index" align='center' width="70"></el-table-column>
				<el-table-column label="项目名称" align="center" sortable="custom" prop="projectName" width="120px" show-overflow-tooltip />
				<el-table-column label="项目简称" align="center" sortable="custom" prop="projectAbbreviation" width="120px" show-overflow-tooltip />
				<el-table-column label="施工单位名称" align="center" sortable="custom" prop="companyName" width="180px" show-overflow-tooltip />
				...
				<el-table-column label="项目地址" align="center" sortable="custom" prop="projectAddress" width="140px" show-overflow-tooltip />
				<!--        <el-table-column :formatter="projectClassifyFormat" label="项目分类" align="center" prop="projectClassify" width="120px"/>-->
				<el-table-column label="总造价（万元）" align="center" sortable="custom" prop="totalCost" width="140px" />
				<el-table-column label="总面积（平方米）" align="center" sortable="custom" prop="totalArea" width="150px" />
				<el-table-column label="开工令日期" align="center" sortable="custom" prop="commencementOrderDate" width="120px">
					<template slot-scope="scope">
						<span>{{ parseTime(scope.row.commencementOrderDate,'{y}-{m}-{d}') }}</span>
					</template>
				</el-table-column>
				<el-table-column label="实际开工日期" align="center" prop="actualCommencementDate" width="140px" sortable="custom">
					<template slot-scope="scope">
						<span>{{ parseTime(scope.row.actualCommencementDate,'{y}-{m}-{d}') }}</span>
					</template>
				</el-table-column>
				<el-table-column label="计划竣工日期" align="center" prop="plannedCompletionDate" width="140px" sortable="custom">
					<template slot-scope="scope">
				...
				<el-table-column :formatter="projectStateFormat" sortable="custom" label="项目状态" align="center" prop="projectState" width="100px" />
			...
			<el-dialog :title="title" :visible.sync="open" width="900px" append-to-body>
				<el-form ref="form" :model="form" :rules="rules" label-width="140px" label-position="right">
					<el-row>
						...
						<el-col :span="12">
							<el-form-item label="统一社会信用代码" prop="uniscid" class="add-form-input">
								<el-input v-model="form.uniscid" placeholder="请输入统一社会信用代码" />
							</el-form-item>
						</el-col>
						<el-col :span="12">
							<el-form-item label="省份" prop="province" class="add-form-input">
								<el-select v-model="form.province" placeholder="请选择" clearable size="small" style="width: 200px" @change="areaChangeProvince">
						...
						<el-col :span="12">
							<el-form-item label="所属市" prop="city" class="add-form-input">
								<el-select v-model="form.city" placeholder="请选择" clearable size="small" style="width: 200px" @change="areaChangeCity">
						...
						<el-col :span="12">
							<el-form-item label="所属区" prop="district" class="add-form-input">
								<el-select v-model="form.district" placeholder="请选择" clearable size="small" style="width: 200px" @change="areaChangeDistrict">
						...
						<el-col :span="12">
							<el-form-item label="工程状态" prop="engineeringStatus" class="add-form-input">
								<el-select v-model="form.engineeringStatus" placeholder="请选择" clearable size="small" style="width: 160px">
						...
						<el-col :span="12">
							<el-form-item label="项目状态" prop="projectState" class="add-form-input">
								<el-select v-model="form.projectState" placeholder="请选择" clearable size="small" style="width: 160px">
									<el-option v-for="dict in projectStateOptions" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
								</el-select>
							</el-form-item>
						</el-col>
						<el-col :span="12">
							<el-form-item label="合同开工日期" prop="contractCommencementDate" class="add-form-input">
								<el-date-picker v-model="form.contractCommencementDate" type="date" placeholder="选择日期" value-format="yyyy-MM-dd HH:mm:ss" />
							</el-form-item>
						</el-col>
						<el-col :span="12">
							<el-form-item label="合同竣工日期" prop="contractCompletionDate" class="add-form-input">
								<el-date-picker v-model="form.contractCompletionDate" type="date" placeholder="选择日期" value-format="yyyy-MM-dd HH:mm:ss" />
							</el-form-item>
						</el-col>
						<el-col :span="24">
							...
							<create-sections style="overflow:hidden" title="施工平面图">
								<el-row>
									<el-col :span="24" style="padding: 10px">
										<wk-upload-img name="file" ref="wkUploadImg3" multiple accept="image/*" list-type="picture-card" :autoUpload="false" :title="'施工平面图'" :batchId="form.planBatchId"></wk-upload-img>
										<!--<el-upload v-if="!systemAddPlaneImage" :show-file-list="false" :http-request="fileAddPlaneUpload" drag class="upload" action="http" accept="image/png, image/jpeg, image/gif, image/jpg">
											<i class="el-icon-plus uploader-icon" />
										</el-upload>
										<div v-else class="upload-show">
											<img v-src="systemAddPlaneImage">
											<i class="el-icon-remove icon-delete" @click="deleteSystemAddPlaneImage" />
										</div>

										<edit-image :fixed-number="[16, 9]" :show="showEditAddPlaneImage" :image="editAddPlaneImage" :file="editAddPlaneFile" title="编辑项目图片" preview-width="160px" preview-height="90px" preview-radius="0" width="550px" save-button-title="确定" @save="submiteAddPlaneImage" @close="showEditAddPlaneImage=false" />-->
									</el-col>
			...
			<el-dialog :title="title" :visible.sync="open2" class="updataopen" width="1200px" append-to-body>
				<el-form ref="form" :model="form" :rules="rules" label-width="170px" class="add-form">
					<create-sections style="overflow:hidden" title="基本信息">
						<el-row>
							<el-col :span="12" class="add-form-w100">
								<el-form-item label="项目名称" prop="projectName" class="add-form-input">
									<el-input v-model="form.projectName" placeholder="请输入项目名称" />
								</el-form-item>
							</el-col>
							...
							<el-col :span="12" class="add-form-w100">
								<el-form-item label="统一社会信用代码" prop="constructionUnitUniscid" class="add-form-input">
									<el-input v-model="form.constructionUnitUniscid" placeholder="请输入建设单统一社会信用代码" />
								</el-form-item>
							</el-col>
							<el-col :span="12" class="add-form-w100">
								<el-form-item label="施工许可证编号" prop="constructionPermitCode" class="add-form-input">
						...
						<el-form-item label="项目分类" prop="projectClassify" class="add-form-input">
							<el-select v-model="form.projectClassify" placeholder="请选择" clearable size="small" style="width: 160px">
								<el-option v-for="dict in projectClassifyOptions" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
							</el-select>
						</el-form-item>
						<el-form-item label="工程用途" prop="engineeringPurpose" class="add-form-input">
							<el-select v-model="form.engineeringPurpose" placeholder="请选择" clearable size="small" style="width: 160px">
								<el-option v-for="dict in engineeringPurposeOptions" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
							</el-select>
						</el-form-item>
						<el-form-item label="建设性质" prop="constructionNature" class="add-form-input">
							<el-select v-model="form.constructionNature" placeholder="请选择" clearable size="small" style="width: 160px">
								<el-option v-for="dict in constructionNatureOptions" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
							</el-select>
						</el-form-item>
						...
						<el-form-item label="实际开工日期" prop="actualCommencementDate" class="add-form-input">
							<el-date-picker v-model="form.actualCommencementDate" type="date" placeholder="选择日期" value-format="yyyy-MM-dd HH:mm:ss" />
						</el-form-item>
						<el-form-item label="实际竣工日期" prop="actualCompletionDate" class="add-form-input">
							<el-date-picker v-model="form.actualCompletionDate" type="date" placeholder="选择日期" value-format="yyyy-MM-dd HH:mm:ss" />
						</el-form-item>
						...
						<el-form-item label="立项级别" prop="createProjectLevel" class="add-form-input">
							<el-select v-model="form.createProjectLevel" placeholder="请选择" clearable size="small" style="width: 160px">
								<el-option v-for="dict in createProjectLevelOptions" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
							</el-select>
						</el-form-item>
						<el-form-item label="建筑规模" prop="constructionScale" class="add-form-input">
							<el-select v-model="form.constructionScale" placeholder="请选择" clearable size="small" style="width: 160px">
								<el-option v-for="dict in constructionScaleOptions" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
							</el-select>
						</el-form-item>
...
<!--							<el-col :span="24" style="padding: 10px ">-->
                ...
                <div
                  v-else
                  class="upload-show">
                  <img style="margin: 0 auto;display: block;height: 33%;width:auto;"   :onerror='`this.src="${require("@/assets/noimgsrc.png")}"`'    v-src="systemImage">
                  <i
...
<!--						</el-row>-->
          ...
          <create-sections>
            <el-row>
              <el-col :span="24" style="padding: 10px ">
                <wk-upload-img name="file" ref="wkUploadImg" multiple accept="image/*" list-type="picture-card" :autoUpload="false" :title="'项目图片上传'" :batchId="form.imgBatchId"></wk-upload-img>
              </el-col>
            </el-row>
          </create-sections>
					...
					<create-sections style="overflow:hidden" prop="corporateAccountBank" title="银行信息">
						<el-form-item label="对公账号开户银行" class="add-form-input">
							<el-select v-model="form.corporateAccountBank" placeholder="请选择" clearable size="small" style="width: 160px">
								<el-option v-for="dict in bankAccountOptions" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
							</el-select>
						</el-form-item>
						...
						<el-form-item label="代发工资银行" prop="payBank" class="add-form-input">
							<el-select v-model="form.payBank" placeholder="请选择" clearable size="small" style="width: 160px">
								<el-option v-for="dict in bankAccountOptions" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
							</el-select>
						</el-form-item>
						<el-form-item label="代发工资银行账号" prop="payBankCode" class="add-form-input">
					...
					<create-sections style="overflow:hidden" title="中标信息">
						...
						<el-form-item label="统一社会信用代码" prop="uniscid" class="add-form-input">
							<el-input v-model="form.uniscid" placeholder="请输入施工单位统一社会信用代码" />
						</el-form-item>
						...
						<el-form-item label="所属国家" prop="country" class="add-form-input">
							<el-select v-model="form.country" placeholder="请选择" clearable size="small" style="width: 160px">
								<el-option v-for="dict in countryOptions" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
							</el-select>
						</el-form-item>
						<el-form-item label="所属区域" prop="region" class="add-form-input">
							<el-select v-model="form.region" placeholder="请选择" clearable size="small" style="width: 160px">
								<el-option v-for="dict in regionOptions" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
							</el-select>
						</el-form-item>
						...
						<el-form-item label="使用资质" prop="qualification" class="add-form-input">
							<el-select v-model="form.qualification" placeholder="请选择" clearable size="small" style="width: 160px">
								<el-option v-for="dict in qualificationOptions" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
							</el-select>
						</el-form-item>
						...
						<el-form-item label="项目类型" prop="projectType" class="add-form-input">
							<el-select v-model="form.projectType" placeholder="请选择" clearable size="small" style="width: 160px">
						...
						<el-form-item label="中标日期" prop="bidWinningDate" class="add-form-input">
							<el-date-picker v-model="form.bidWinningDate" type="date" placeholder="选择日期" prop="constructionUnitName" value-format="yyyy-MM-dd HH:mm:ss" />
						</el-form-item>
						<el-form-item label="中标开工日期" prop="bidWinningStartDate" class="add-form-input">
							<el-date-picker v-model="form.bidWinningStartDate" type="date" placeholder="选择日期" value-format="yyyy-MM-dd HH:mm:ss" />
						</el-form-item>
						<el-form-item label="中标竣工日期" prop="bidWinningCompletionDate" class="add-form-input">
							<el-date-picker v-model="form.bidWinningCompletionDate" type="date" placeholder="选择日期" value-format="yyyy-MM-dd HH:mm:ss" />
						</el-form-item>
						...
						<el-form-item label="承包模式" prop="contractMode" class="add-form-input">
							<el-select v-model="form.contractMode" placeholder="请选择" clearable size="small" style="width: 160px">
								<el-option v-for="dict in contractModeOptions" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
							</el-select>
						</el-form-item>
						<el-form-item label="高达精特" prop="tallExquisite" class="add-form-input">
							<el-select v-model="form.tallExquisite" placeholder="请选择" clearable size="small" style="width: 160px">
								<el-option v-for="dict in tallExquisiteOptions" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
							</el-select>
						</el-form-item>
						...
						<el-form-item label="经营模式" prop="managementModel" class="add-form-input">
							<el-select v-model="form.managementModel" placeholder="请选择" clearable size="small" style="width: 160px">
								<el-option v-for="dict in managementModelOptions" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
							</el-select>
						</el-form-item>
						<el-form-item label="分包模式" prop="subcontractingMode" class="add-form-input">
							<el-select v-model="form.subcontractingMode" placeholder="请选择" clearable size="small" style="width: 160px">
								<el-option v-for="dict in subcontractingModeOptions" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
							</el-select>
						</el-form-item>
					</create-sections>
					<create-sections style="overflow:hidden" title="工程信息">
						...
						<el-form-item label="项目级别" prop="projectLevel" class="add-form-input">
							<el-select v-model="form.projectLevel" placeholder="请选择" clearable size="small" style="width: 160px">
								<el-option v-for="dict in projectLevelOptions" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
							</el-select>
						</el-form-item>
						...
						<el-form-item label="开工令日期" prop="commencementOrderDate" class="add-form-input">
							<el-date-picker v-model="form.commencementOrderDate" type="date" placeholder="选择日期" value-format="yyyy-MM-dd HH:mm:ss" />
						</el-form-item>
						<el-form-item label="计划竣工日期" prop="plannedCompletionDate" class="add-form-input">
							<el-date-picker v-model="form.plannedCompletionDate" type="date" placeholder="选择日期" value-format="yyyy-MM-dd HH:mm:ss" />
						</el-form-item>
						...
						<el-form-item label="工程类别" prop="engineeringType" class="add-form-input">
							<el-select v-model="form.engineeringType" placeholder="请选择" clearable size="small" style="width: 160px">
								<el-option v-for="dict in engineeringTypeOptions" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
							</el-select>
						</el-form-item>
					...
					<create-sections style="overflow:hidden" title="质量安全精品创建">
						<el-form-item label="质量目标" class="add-form-input">
							<el-select v-model="form.qualityTarget" placeholder="请选择" clearable size="small" style="width: 160px">
								<el-option v-for="dict in qualityTargetOptions" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
							</el-select>
						</el-form-item>
						<el-form-item label="安全目标" class="add-form-input">
							<el-select v-model="form.safetyTarget" placeholder="请选择" clearable size="small" style="width: 160px">
								<el-option v-for="dict in safetyTargetOptions" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
							</el-select>
						</el-form-item>
						<el-form-item label="创建市优质结构" class="add-form-input">
							<el-select v-model="form.qualityStructure" placeholder="请选择" clearable size="small" style="width: 160px">
								<el-option v-for="dict in yesNoOptions" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
							</el-select>
						</el-form-item>
						<el-form-item label="观感质量等级目标" class="add-form-input">
							<el-select v-model="form.qualityLevelTarget" placeholder="请选择" clearable size="small" style="width: 160px">
								<el-option v-for="dict in qualityLevelTargetOptions" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
							</el-select>
						</el-form-item>
					...
					<create-sections style="overflow:hidden" title="施工平面图">
						<el-row>
							<el-col :span="24" style="padding: 10px">
								<wk-upload-img name="file" ref="wkUploadImg2" multiple accept="image/*" list-type="picture-card" :autoUpload="false" :title="'施工平面图'" :batchId="form.planBatchId"></wk-upload-img>
								<!--<el-upload v-if="!systemAddPlaneImage" :show-file-list="false" :http-request="fileAddPlaneUpload" drag class="upload" action="http" accept="image/png, image/jpeg, image/gif, image/jpg">
									<i class="el-icon-plus uploader-icon" />
								</el-upload>
								<div v-else class="upload-show">
									<img :onerror='`this.src="${require("@/assets/noimgsrc.png")}"`' v-src="systemAddPlaneImage">
									<i class="el-icon-remove icon-delete" @click="deleteSystemAddPlaneImage" />
								</div>
					...
					<el-row>
						...
						<el-col :span="12">
							<el-form-item label="是否公共区域" prop="isPublic" class="add-form-input">
								<el-select v-model="lzform.isPublic" placeholder="请选择" clearable size="small" style="width: 160px">
									<el-option v-for="dict in yesNoOptions" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
								</el-select>
							</el-form-item>
						</el-col>
						<el-col :span="12">
							<el-form-item label="类型" prop="type" class="add-form-input">
								<el-select v-model="lzform.type" placeholder="请选择" clearable size="small" style="width: 160px">
									<el-option v-for="dict in SingleBuildingType" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
								</el-select>
							</el-form-item>
						</el-col>
						<el-col :span="12">
...
<script>
	import { syncAccountApi, getList, addProjectInfo, updateProjectInfo, getProjectInfoById, delSupplyPoint, addBuilding, selectById, selectByProjectId, removeById } from '@/api/build/projectInfo'
	...
	export default {
		...
		mounted() {
			console.log(this.$route.query.monitorType)
			// this.queryParams.monitorType=this.$route.query.monitorType
			this.$route.query.projectState != undefined && this.$route.query.projectState != '' ?
				this.$set(this.queryParams, 'projectState', this.$route.query.projectState) : ''
		},
		methods: {
			//同步账号
			syncAccount() {
				this.loading = true;
				syncAccountApi().then(response => {
			...
			handler({
				...
				this.center.lng = 117.323065
				this.center.lat = 34.806747
				this.zoom = 12
			},
			/** 查询字典类型列表 */
			getList() {
			...
			handleAdd() {
				this.reset()
				this.form.imgBatchId = guid()
				this.form.planBatchId = guid()
				this.open = true
				this.title = '添加项目信息'
			},
			/** 修改按钮操作 */
			handleUpdate(row) {
				...
				getProjectInfoById(id).then(response => {
					...
					this.form.imgBatchId = this.form.imgBatchId ? this.form.imgBatchId : guid()
					this.form.planBatchId = this.form.planBatchId ? this.form.planBatchId : guid()
				...
				this.$refs['form'].validate(valid => {
					if(valid) {
						if(this.form.id != undefined) {
							...
							var fileFlag = this.$refs['wkUploadImg3'].submitAsync();
							  if (!fileFlag) {
								  this.$message({
									  type: 'error',
									  message: '施工平面图上传异常'
			...
			fileUpload(content) {
				...
				reader.onload = function(e) {
					...
					self.editedImage = content
					self.systemImage = result
			...
			fileAddPlaneUpload(content) {
				...
				reader.onload = function(e) {
					...
					self.editedAddPlaneImage = content
					self.systemAddPlaneImage = result
