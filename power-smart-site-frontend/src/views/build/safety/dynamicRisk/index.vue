  ...
  <div class="main">
    ...
    <div class="main-body">
      ...
      <el-table v-loading="loading" :height="tableHeight" :data="dataList" @sort-change="sortChange">
        ...
        <el-table-column label="风险点描述" align="center" prop="riskDesc" :show-overflow-tooltip="true" sortable="custom" />
...
<!--        <el-table-column label="风险解除状态" :formatter="relieveStatusFc" align="center" prop="relieveStatus" :show-overflow-tooltip="true" />-->
      ...
      <!-- 新增或修改参数配置对话框 -->
      <el-dialog :title="title" :visible.sync="open" width="900px"   @close="handleClose">
        <el-form ref="form"  :model="form" :rules="rules" label-width="100px" class="add-form">
          <el-row>
            ...
            <el-col :span="12">
              <el-form-item label="风险点描述" prop="riskDesc" class="add-form-input">
            ...
            <el-col :span="6">
              <el-form-item label="风险评价L" prop="riskAssessmentL" class="add-form-input">
                <el-input v-model="form.riskAssessmentL" placeholder="风险评价L" />
              </el-form-item>
            </el-col>
            <el-col :span="6">
              <el-form-item label="风险评价E" prop="riskAssessmentE" class="add-form-input">
                <el-input v-model="form.riskAssessmentE" placeholder="风险评价E" />
              </el-form-item>
            </el-col>
            <el-col :span="6">
              <el-form-item label="风险评价C" prop="riskAssessmentC" class="add-form-input">
                <el-input v-model="form.riskAssessmentC" placeholder="风险评价C" />
              </el-form-item>
            </el-col>
            <el-col :span="6">
              <el-form-item label="风险评价D" prop="riskAssessmentD" class="add-form-input">
                <el-input v-model="form.riskAssessmentD" placeholder="风险评价D" />
              </el-form-item>
            </el-col>
          </el-row>
        </el-form>
...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        this.$refs['form'].validate(valid => {
          if (valid) {
            if (this.form.id != undefined) {
              request({
                url: 'build/safetyDynamicRisk/edit',
                method: 'post',
                data: this.form,
                headers: {
                  'Content-Type': 'application/json;charset=UTF-8'
              ...
              request({
                url: 'build/safetyDynamicRisk/add',
                method: 'post',
                data: this.form,
                headers: {
                  'Content-Type': 'application/json;charset=UTF-8'
      ...
      handleAdd () {
		request({
			url: 'build/safetyDynamicRisk/generateDocumentNumber',
			method: 'post'
		}).then(response => {
			this.reset()
      ...
      handleUpdate (row) {
        ...
        request({
          url: 'build/safetyDynamicRisk/selectById/' + pipeId,
          method: 'get'
        }).then(response => {
      ...
      handleDelete (row) {
        ...
        this.$confirm('是否确认删除该数据项?', '警告', {
          ...
          return request({
            url: 'build/safetyDynamicRisk/deleteDetails/' + id,
            method: 'get'
          })
        }).then(() => {
          this.getList()
      ...
      bulkImportClick() {
        ...
        let dataLists = [
          ...
          "collapse",
          "0",
          "编制专项施工方案、对开挖深度范围内有地下水的必须设置井点降水",
          "开挖过程中及开挖结束后检查坑底有无明水",
          "加强安全教育",
          "加强安全巡查",
          "现场设置应急抢险物资",
        ...
        this.param = {
          title:"作业活动风险管理(*)为必填项",
          excelTitle:"作业活动风险管理模板",
          colNames:lists,
          sampleData:dataLists,
          url:'build/safetyDynamicRisk/excelImport'
        }
