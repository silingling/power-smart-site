...
<script>
  ...
  import {getList, delTrueByIds} from "../api/inspecitonPlan"
  ...
  export default {
    ...
    methods: {
      ...
      fieldFormatter(row, column, cellValue) {
		  if (column.property === 'planType') {
		  	return this.selectDictLabel(this.dictData.planTypeDict, row.planType)
		  } else if (column.property === 'planLevel') {
		  	return this.selectDictLabel(this.dictData.planLevelDict, row.planLevel)
		  } else if (column.property === 'cycleUnit') {
	  ...
	  // 初始化字典数据
	  initDicts() {
	  	this.getDicts('b_safety_plan_type').then(response => {
	  	  this.dictData.planTypeDict = response.data
	  	})
	  	this.getDicts('b_safety_cycle_unit').then(response => {
