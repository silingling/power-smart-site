        ...
        <el-col :span="4"  style="">
          <el-card class="box-card" style="margin: 10px;height:calc(100vh - 170px);">
            <div slot="header" class="clearfix">
              <span style="font-weight: bold;font-size: 16px">安全资料目录</span>
          ...
          <el-card class="box-card" style="margin: 10px;height:calc(100vh - 170px);">
            ...
            <el-tabs v-model="activeName" @tab-click="handleClick" :height="tableHeight">
              ...
              <el-tab-pane label="已收藏" name="collect">
                <el-table v-loading="loading" :data="dataCollectList" >
                  <el-table-column label="资料编号" align="center">
                    <template slot-scope="scope">
...
<script>
  import { getList, addCatalog, getCatalogTree ,deleteCatalog,deleteOneselfAndSublevel,getCollectList,get,removeById,collect,deleteCollect,selectById } from '@/api/build/safetyMaterial'
    ...
    methods: {
      init(){
        getCatalogTree(0).then(response => {
		  this.catalogData = [{id:0,label:'全部目录'}]
      ...
      remove(data,node) {
        ...
        this.$confirm(content, '提示', {
          ...
          deleteOneselfAndSublevel(data.id).then(response => {
            console.log(response)
            if (response.code === 0) {
              this.msgSuccess('删除成功')
              this.init();
			  
			  // 列表数据是删除目录的数据，则查询全部数据
			  if (this.catalogId == data.id) {
				  this.catalogId = 0
				  this.getList()
			  }
            }
