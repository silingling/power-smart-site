    ...
    <el-form v-show="true" ref="queryForm" :model="queryParams" :inline="true" label-width="80px"
      ...
      <el-form-item label="库房名称" prop="roomName">
        ...
        />
         ...
         <el-form-item label="监测项目" prop="monitorPro" >
              <el-select
                    v-model="queryParams.monitorPro"
                    placeholder="请选择"
                    clearable
                    size="small"
                    style="width: 150px">
              <el-option
                      v-for="dict in roomInfoMonitorPro"
                      :key="dict.dictValue"
       ...
       <el-form-item label="报警类型" prop="alertType" >
              <el-select
                    ...
                    style="width: 150px">
              <el-option
                      v-for="dict in roomInfoAlertType"
                      :key="dict.dictValue"
                      :label="dict.dictLabel"
                      :value="dict.dictValue" />
            </el-select>
      ...
      <el-form-item style="float: right">
        <div class="right-container">
          <el-button class="export-btn" type="primary" style="float:right;margin-top: 4px;margin-right: 20px;" @click="excelImport">导入</el-button>
          <!-- <el-button
            v-if="canHandle('add')"
            class="xr-btn--orange"
            icon="el-icon-plus"
...
//   import Create from './Create'
  ...
  import {getList} from "../api/roomInfoAlert";
  ...
  export default {
    ...
    data() {
      return {
       ...
       //数据字典
        roomInfoMonitorPro:[],//监测项目
        roomInfoAlertType:[],//报警类型
        //导入
    ...
    mounted() {
      ...
      this.getList();
      //获取监测项目字典
      this.getDicts('b_safety_roomInfo_monitor_pro').then(response => {
      this.roomInfoMonitorPro = response.data
    })
    //获取报警类型字典
      this.getDicts('b_safety_roomInfo_alert_type').then(response => {
      this.roomInfoAlertType = response.data
    })
    ...
    methods: {
      ...
      //导入
        ...
        this.param = {
          ...
          url:'build/safetyRoomInfoAlert/excelImport'
        }
      ...
      getList() {
		...
		// 报警时间范围
		if(this.dispatchTime != null && this.dispatchTime != undefined && this.dispatchTime != ''){
			this.queryParams.alarmTimeStart = this.dispatchTime[0]
			this.queryParams.alarmTimeEnd = this.dispatchTime[1]
		}else{
      ...
      fieldFormatter(row, column, cellValue) {
        ...
        if(column.property=="monitorPro"){
          return this.selectDictLabel(this.roomInfoMonitorPro, row.monitorPro)
        } else if(column.property=="alertType"){
          return this.selectDictLabel(this.roomInfoAlertType, row.alertType)
        }else{
          return  row[column.property] === '' || row[column.property] === null ? '--' : row[column.property]
        }
