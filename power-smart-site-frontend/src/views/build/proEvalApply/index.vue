    ...
    <div class="main-body">
      <el-form v-show="showSearch" ref="queryForm" :model="queryParams" :scroll-wheel-zoom="true" :inline="true" label-width="80px" style="padding-top: 20px;padding-left:10px;">
        ...
        <el-form-item label="项目状态" prop="projectState">
          ...
          >
            <el-option
              v-for="dict in projectStateOptions"
              :key="dict.dictValue"
      ...
      <el-table :cell-class-name="cellClassName" :height="tableHeight" @row-click="handleRowClick" v-loading="loading" v-if="isList" :data="typeList" @sort-change="sortChange">
        <el-table-column label="序号" type="index" align='center' width="60"></el-table-column>
        <el-table-column label="项目名称" align="center" prop="projectName" show-overflow-tooltip width="120px" sortable="custom"/>
        <el-table-column label="项目简称" align="center" prop="projectAbbreviation" width="120px" show-overflow-tooltip sortable="custom"/>
        <el-table-column label="施工单位名称" align="center" prop="companyName" width="180px" sortable="custom"/>
        ...
        <el-table-column label="项目地址" align="center" prop="projectAddress" show-overflow-tooltip width="120px"  sortable="custom"/>
        <el-table-column label="工地评级" align="center" prop="appLevel" width="120px" sortable="custom"/>
<!--        <el-table-column :formatter="projectClassifyFormat" label="项目分类" align="center" prop="projectClassify" width="120px"/>-->
        <el-table-column label="总造价（万元）" align="center" prop="totalCost" width="140px" sortable="custom"/>
        <el-table-column label="总面积（平方米）" align="center" prop="totalArea" width="120px"/>
        <el-table-column label="开工令日期" align="center" prop="commencementOrderDate" width="120px" sortable="custom">
...
<!--        <el-table-column :formatter="engineeringStatusFormat" label="工程状态" align="center" prop="engineeringStatus"/>-->
        <el-table-column :formatter="projectStateFormat" label="项目状态" align="center" prop="projectState" width="100px" sortable="custom"/>
<!--        <el-table-column label="单位工程数量" align="center" prop="unitQuantity" />-->
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="200px">
          <template slot-scope="scope">
            ...
            >修改</el-button> -->
			<el-button
			  v-if="scope.row.score < 90"
			  size="mini"
			  type="text"
			  icon="el-icon-edit"
			  @click="evalApplyClick(scope.row.id)"
			>评级申请</el-button>
			<el-button
			  size="mini"
			  type="text"
			  icon="el-icon-document"
			  @click="evalDetailsClick(scope.row.evalApplyId)"
			>评级详情</el-button>
          </template>
        </el-table-column>
      </el-table>
...
<script>
import { getListByEval, addProjectInfo, updateProjectInfo, getProjectInfoById, delSupplyPoint, addBuilding, selectByProjectId, removeById, selectLevelList } from '@/api/build/projectInfo'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getListByEval(this.queryParams).then(response => {
	 ...
	 * 评级详情
	 */
	evalDetailsClick(evalApplyId) {
	    this.action.type = 'detail'
	    this.action.id = evalApplyId
          ...
          if (this.form.id != undefined) {
            if (this.editedImage != null) {
              if(this.editedAddPlaneImage != null){
                var batchId = guid()
                var data = { type: 'img', batchId: batchId, file: this.editedImage.blob }
                crmFileSaveAPI(data).then(response => {
                  if (response.code === 0) {
                    this.form.projectImg = response.data.url
                    var batchId2 = guid()
                    var data2 = { type: 'img', batchId: batchId2, file: this.editedAddPlaneImage.blob }
                    crmFileSaveAPI(data2).then(response => {
                      if (response.code === 0) {
	...
	handleRowClick(row, column, event) {
	  if (column.property == 'appLevel') {
	    this.action.type = 'detail'
	    this.action.id = row.evalApplyId
	    this.detailShow = true;
	  }
	},
