	...
	<div class="main">
		...
		<div class="main-body">
			...
			<el-table v-loading="loading" :height="tableHeight" :data="dataList" @sort-change="sortChange">
				<el-table-column label="检查评定项目" align="center" prop="securityEvaluationId"
				...
				<el-table-column label="检查子项" align="center" prop="securityEvaluationSubitem" sortable="custom" />
				<el-table-column label="扣分标准" align="center" prop="securityEvaluationStandard" show-overflow-tooltip sortable="custom" />
			...
			<el-dialog :title="title" :visible.sync="open" width="900px" append-to-body>
				<el-form ref="form" :model="form" :rules="rules" label-width="100px" class="add-form">
					<el-row>
						<el-col :span="24">
							<el-form-item label="检查评定项目" class="add-form-input label_1" prop="securityEvaluationId">
								<el-select v-model="form.securityEvaluationId" placeholder="请选择" clearable size="small"
									style="width: 160px">
									<el-option v-for="dict in securityEvaluationOptions" :key="dict.dictValue"
										:label="dict.dictLabel" :value="dict.dictValue" />
								</el-select>
						...
						<el-col :span="12">
							<el-form-item label="检查项目"  class="add-form-input">
								<el-input v-model="form.securityEvaluationName" placeholder="请输入检查项目" />
							</el-form-item>
						</el-col>
						<el-col :span="12">
							<el-form-item label="检查子项" prop="securityEvaluationSubitem" class="add-form-input">
								<el-input v-model="form.securityEvaluationSubitem" placeholder="请输入检查项目" />
							</el-form-item>
						</el-col>
					</el-row>
					<el-row>
	...
	export default {
		...
		methods: {
			...
			getList() {
				...
				request({
					url: 'build/bProgressSecurity/list',
					method: 'post',
			...
			handleUpdate(row) {
				...
				request({
					url: 'build/bProgressSecurity/selectById/' + pipeId,
					method: 'get'
				}).then(response => {
				...
				this.$refs['form'].validate(valid => {
					if (valid) {
						if (this.form.id != undefined) {
							request({
								url: 'build/bProgressSecurity/edit',
								method: 'post',
								data: this.form,
								headers: {
									'Content-Type': 'application/json;charset=UTF-8'
							...
							request({
								url: 'build/bProgressSecurity/add',
								method: 'post',
								data: this.form,
								headers: {
									'Content-Type': 'application/json;charset=UTF-8'
			...
			handleDelete(row) {
				...
				this.$confirm('是否确认删除该数据项?', '警告', {
					...
					return request({
						url: 'build/bProgressSecurity/deleteDetails/' + id,
						method: 'get'
					})
				}).then(() => {
					this.getList()
			...
			securityEvaluationFormat(row, column) {
				return this.selectDictLabel(this.securityEvaluationOptions, row.securityEvaluationId)
			},
			/**
			 * 批量导入
			 */
			bulkImportClick() {
				...
				this.param = {
					title: "建筑施工检查标准管理(*)为必填项",
					excelTitle: "建筑施工检查标准管理模板",
					colNames: lists,
					sampleData: dataLists,
					url: 'build/bProgressSecurity/excelImport'
				}
