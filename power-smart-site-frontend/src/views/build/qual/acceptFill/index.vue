  ...
  <div>
    <el-form v-show="true" ref="queryForm" :model="queryParams" :inline="true" label-width="80px" style="padding: 20px 20px 0px 20px;background-color: white;border-top: 0px solid #e6e6e6;">
     <!-- <el-form-item label="模板名称" prop="templateName">
        <el-input v-model="queryParams.templateName" placeholder="请输入验收模板名称" clearable size="small" @keyup.enter.native="getList" />
      </el-form-item>-->
      <el-form-item label="验收工序" prop="acceptWork">
        <el-select v-model="queryParams.acceptWork" placeholder="请选择" clearable size="small" style="width: 100%">
          <el-option v-for="dict in acceptWorks" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
        </el-select>
      </el-form-item>
      <el-form-item label="户型" prop="houseId">
        <el-select v-model="queryParams.houseId" placeholder="请选择" clearable size="small" style="width: 100%">
          <el-option v-for="dict in houseTemplateNameList" :key="dict.id" :label="dict.houseTemplateName" :value="dict.id" />
        </el-select>
      </el-form-item>
      <el-form-item label="验收时间" prop="acceptWork">
        <el-date-picker
          class="timecom"
          v-model="sjtbDate"
          type="daterange"
          ...
          unlink-panels
          range-separator="至"
          start-placeholder="验收时间起"
          end-placeholder="验收时间止"
          :picker-options="pickerOptions">
        </el-date-picker>
      </el-form-item>
      <el-form-item label="是否通过" prop="isAdopt">
        <el-select v-model="queryParams.isAdopt" placeholder="请选择" clearable size="small" style="width: 100%">
      ...
      <el-form-item style="float: right">
        <div class="right-container">
          <el-button v-if="canHandle('add')" class="xr-btn--orange" icon="el-icon-plus" type="primary" @click="createClick">新建 </el-button>
          <!-- <el-dropdown v-if="moreTypes.length > 0" trigger="click" @command="handleTypeDrop">
            <el-button icon="el-icon-more" />
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item v-for="(item, index) in moreTypes" :key="index" v-if="canHandle(item.type)" :icon="item.icon | wkIconPre" :command="item.type">{{ item.name }}
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown> -->
        </div>
    ...
    <div v-empty="!canHandle('list')" xs-empty-icon="nopermission" xs-empty-text="暂无权限">
      <el-table v-loading="loading" id="crm-table" :data="list" :height="tableHeight" :cell-class-name="cellClassName" :header-cell-class-name="headerCellClassName" class="n-table--border" use-virtual stripe border highlight-current-row style="width: 100%" @row-click="handleRowClick" @sort-change="sortChange" @selection-change="handleSelectionChange">
        ...
        <el-table-column v-for="(item, index) in fieldList" :key="index" :prop="item.prop" :label="item.label" :width="item.width" :formatter="fieldFormatter" sortable="custom" show-overflow-tooltip></el-table-column>
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="200px">
          <template slot-scope="scope">
            <el-button size="mini" type="text" icon="el-icon-document" @click="showDetails(scope.row.acceptId)">查看</el-button>
            <el-button size="mini" type="text" icon="el-icon-edit" @click="updateClick(scope.row.acceptId)">修改 </el-button>
            <el-button size="mini" type="text" icon="el-icon-delete" @click="deleteClick(scope.row.acceptId)">删除 </el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 相关详情页面 -->
    <detail v-if="showDview" :businessType="businessType" :titleName="titleName" :action="action" @close="showDview = false" />
    <!-- 新建 -->
    <create v-if="createShow" :businessType="businessType" :titleName="titleName" :action="action" @close="createShow = false" @save-success="getList" />
  </div>
</template>
...
import { queryList, del, buildList, TempLateSetList, getAllHouseList,gethouseTemplateList } from "../api/acceptFill";
...
export default {
  ...
  data () {
    return {
      ...
      acceptWorks: [],//验收工序
      buildList: [],//楼座列表
      TemplateSetList: [],//模板设置列表
      houseList:[],
      // acceptConclusion: [{ 'dictLabel': '通过', 'dictValue': '0' }, { 'dictLabel': '不通过', 'dictValue': '1' }],//验收结论
  ...
  created () {
    ...
    TempLateSetList().then(response => {
      this.TemplateSetList = response.data.list;
    })
    getAllHouseList().then(response => {
      // console.log(response)
      this.houseList = response.data;
    })
    // 获取所有户型
    gethouseTemplateList().then(res => {
 ...
 //初始化数据
    ...
    getList () {
      ...
      if (this.sjtbDate != null&&this.sjtbDate != '') {
        ...
        d = d < 10 ? ('0' + d) : d
        const time = y + '-' + m + '-' + d
        this.queryParams.locationLon = time
        ...
        d = d < 10 ? ('0' + d) : d
        const time2 = y + '-' + m + '-' + d
        this.queryParams.locationLat = time2
      }else{
      	this.queryParams.locationLon = ''
      	this.queryParams.locationLat = ''
      }
      //分页
    ...
    handleTypeDrop (command) {
      if (command == 'delete') {//删除
        this.$confirm("确定删除选中的数据吗？", '提示', {
          ...
          .then(() => {
            const ids = this.selectionList.map(item => item['acceptId'])
            this.loading = true;
            del(ids).then(res => {
              if (res.code == 0) {
    ...
    fieldFormatter (row, column, cellValue) {
      if (column.property === 'acceptWork') {
        return this.selectDictLabel(this.acceptWorks, row.acceptWork)
      } else if (column.property === 'buildingId') {
        return this.CustomFormat(this.buildList, row.buildingId)
      } else if (column.property === 'templateNameId') {
        return this.CustomFormat2(this.TemplateSetList, row.templateNameId)
      } else if (column.property === 'houseId') {
        return this.CustomFormat(this.houseList,row.houseId)
        // return this.selectDictLabel(this.acceptConclusion, row.acceptConclusion + '')
      }else if(column.property === 'acceptBy'){// 验收人员
        return this.acceptByFormat(this.acceptByList,row.acceptBy)
      } else if (column.property === 'isAdopt') {// 是否通过
        var tg="";
        if (row.isAdopt == '0') {
          tg = "否";
        ...
        return tg
      } else if (column.property === 'acceptTime') { // 验收时间
    ...
    CustomFormat (datas, value) {
      ...
      Object.keys(datas).some(key => {
        if (datas[key].buildingId == "" + value) {
          actions.push(datas[key].buildingName);
          return true;
        }
      });
      return actions.join("");
