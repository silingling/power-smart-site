<template>
  <div>
    ...
    <el-form v-show="true" ref="queryForm" :model="queryParams" :inline="true" label-width="80px" style="padding: 20px 20px 0px 20px;background-color: white;border-top: 0px solid #e6e6e6;">
      <el-form-item label="模板名称" prop="checkTemplateName">
        <el-input v-model="queryParams.checkTemplateName" placeholder="请输入模板名称" clearable size="small" @keyup.enter.native="getList" />
      </el-form-item>
      <el-form-item label="检查类型" prop="checkType">
        <el-select v-model="queryParams.checkType" placeholder="请选择" clearable size="small" style="width: 200px">
          <el-option v-for="dict in checkTypes" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
        </el-select>
      </el-form-item>
      <el-form-item label="满分" prop="fullMarks">
        <el-input v-model="queryParams.fullMarks" placeholder="请输入满分" clearable size="small" @keyup.enter.native="getList" />
      </el-form-item>
      <el-form-item>
...
import { getList, remove } from "../api/enterpriseQualityCheckSetting";
import { sortColumn } from '@/utils/index';

export default {
  /** DEMO列表 */
  name: "enterpriseQualityCheckSetting",
  components: {
    Create,
    // Detail
  },
  ...
  methods: {
    ...
    fieldFormatter (row, column, cellValue) {
      ...
      if (column.property == "checkType") {
        return this.selectDictLabel(this.checkTypes, row.checkType);
      } else {
