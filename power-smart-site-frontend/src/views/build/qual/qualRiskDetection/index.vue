  ...
  <div>
    ...
    <el-form v-show="true" ref="queryForm" :model="queryParams" :inline="true" label-width="80px"
	  ...
	  <el-form-item label="数据类型" prop="dataType">
	    <el-select v-model="queryParams.dataType" clearable placeholder="请选择">
	      <el-option
	  ...
	  <el-form-item label="问题级别" prop="problemLevel">
	    <el-select v-model="queryParams.problemLevel" clearable placeholder="请选择">
	      <el-option
	...
	<el-dialog title="选择数据类型" width="20%" :visible.sync="dialogTableVisible">
...
<script>
  ...
  import {getList, delByIds, getSubcontractorList} from "../api/qualRiskDetection"
  ...
  export default {
    ...
    methods: {
      ...
      fieldFormatter(row, column, cellValue) {
		  if (column.property === 'problemLevel') {
		  	return this.selectDictLabel(this.dictData.problemLevelDict, row.problemLevel)
		  } else if (column.property === 'urgency') {
		  	return this.selectDictLabel(this.dictData.urgencyDict, row.urgency)
		  } else if (column.property === 'dataType') {
		  	return this.selectDictLabel(this.dataTypeList, row.dataType)
	  ...
	  // 初始化字典数据
	  initDicts() {
	  	this.getDicts('b_qual_problem_level').then(response => {
	  	  this.dictData.problemLevelDict = response.data
	  	})
	  	this.getDicts('b_qual_urgency').then(response => {
	  	  this.dictData.urgencyDict = response.data
	  	})
	  	this.getDicts('b_qual_inspection_type').then(response => {
	  	  this.dictData.inspectionTypeDict = response.data
	  	})
	  },
	  // 查询分包商
