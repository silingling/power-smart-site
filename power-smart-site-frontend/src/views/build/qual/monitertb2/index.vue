  ...
  <div>
    ...
    >
      <el-table
        ...
        style="width: 100%"
        @row-click="infocollectionSelect"
...
<script>
  ...
  import {getList, delTrueByIds ,getDetailsByZHId} from "../api/moniterTB";
    ...
    methods: {
      ...
      initData(to) {
        ...
        if(to.path.indexOf('/TBstandardroomMonit')>0){
          ...
          this.monitType = '1'
        }else if(to.path.indexOf('/TBinspectMonit')>0){
          this.monitType = '3'
        }
        this.getList()
      },
        ...
        getList(this.queryParams)
          .then(res => {
            ...
            this.$nextTick(() => {
              ...
              // console.log("===="+JSON.stringify(this.$refs.multipleTable.data[0]))
              if(this.$refs.multipleTable.data.length>0){
                this.$refs.multipleTable.setCurrentRow(this.$refs.multipleTable.data[0]);
                this.infocollectionSelect(this.$refs.multipleTable.data[0]);
              }
            })
            this.loading = false
          })
      ...
      infocollectionSelect(row) {
        getDetailsByZHId(row.id,this.monitType).then(res =>{
          this.list2 = res.data
        })
      }
    }
