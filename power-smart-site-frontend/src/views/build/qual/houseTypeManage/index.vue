<template>
  <div>
    ...
    <el-form v-show="true" ref="queryForm" :model="queryParams" :inline="true" label-width="80px" style="padding: 20px 20px 0px 20px;background-color: white;border-top: 0px solid #e6e6e6;">
      <el-form-item label="户型模板名称" prop="houseTemplateName" label-width="100px">
        <el-input v-model="queryParams.houseTemplateName" placeholder="请输入户型模板名称" clearable size="small" @keyup.enter.native="getList" />
      </el-form-item>
      <el-form-item label="验收模板名称" prop="templateName" label-width="100px">
      ...
      <el-table v-loading="loading" id="crm-table" :data="list" :height="tableHeight" :cell-class-name="cellClassName" :header-cell-class-name="headerCellClassName" class="n-table--border" use-virtual stripe border highlight-current-row style="width: 100%" @row-click="handleRowClick" @sort-change="sortChange" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width" fixed="right" width="250px">
          <template slot-scope="scope">
            <el-button size="mini" type="text" icon="el-icon-edit" @click="distribClick(scope.row.id)">分配
            </el-button>
            ...
            <el-button size="mini" type="text" @click="CopyClick(scope)">复制
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 分配 -->
    <distrib v-if="distribShow" :businessType="businessType" :action="action" :titleName="titleName" @close="distribShow = false" @save-success="getList" />
    <!-- 复制节点菜单 -->
    <CopyMenu v-if="showCopyMenu" :menu="currentMenu" :titleName="titleName" :CopyName="showCopyName" @save="getList" @hiden-view="showCopyMenu=false" />
  </div>
</template>
...
import { getList, remove, getById } from "../api/houseTypeManage";
...
export default {
  ...
  methods: {
    //复制菜单
    async CopyClick (scope) {
      this.loading = true;
      await getById(scope.row.id)
        .then(res => {
          this.currentMenu = res.data;
          this.loading = false;
        })
      this.showCopyName = scope.row.houseTemplateName
