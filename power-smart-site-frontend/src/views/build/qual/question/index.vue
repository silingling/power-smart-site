	...
	<div class="main">
		...
		<div class="main-body">
			<div class="main-table-header">
				<el-button class="main-table-header-button xr-btn--orange" type="primary" icon="el-icon-plus" @click="addMenu">新建</el-button>
			...
			<el-table ref="menuTable" :data="menuData" style="width: 100%" :row-key="rowKey" border lazy :load="getChildMenuList"
			 :tree-props="{children: 'children', hasChildren: 'hasChildren'}" :expand-row-keys="expands" @expand-change="expandChange">
				...
				<el-table-column prop="questionName" label="质量问题">
				</el-table-column>
				...
				<el-table-column prop="remarks" align="center" label="备注说明" width="250">
				</el-table-column>
		...
		<!-- 新增下级节点 -->
		<CreateNextMenu v-if="showCreateNextMenu" :pquestionId="currentPid" @save="reloadSelf" @hiden-view="showCreateNextMenu=false" />

		<!-- 编辑质量问题 -->
		<EditMenu v-if="showEditMenu" :menu="currentMenu" @save="reloadParent" @hiden-view="showEditMenu=false" />

		<!-- 批量导入 -->
		<bulk-import-user :show="bulkImportShow" :param="param" @close="reloadImport" />
	</div>
	...
	export default {
		...
		methods: {
			...
			getChildMenuList(tree, treeNode, resolve) {
				queryByParentId(tree.questionId)
					.then(res => {
						//console.log(res)
						resolve(res.data);
					})
			...
			reloadSelf() { //本级重载
				queryByParentId(this.currentMenu.questionId)
					.then(res => {
						//console.log(res)
						this.reloadNode(this.currentMenu.questionId, res.data)
					})
					.catch(() => {
						this.loading = false
					})
			...
			async handleClick(event, scope) {
				//此处需同步调用
				await queryById(scope.row.questionId)
					.then(res => {
						this.currentMenu = res.data;
					})
				// this.currentMenu = scope.row;
				this.currentPid = this.currentMenu.questionId;
				if (event == 'next') { //创建下级
					...
					this.$confirm('您确定要删除吗?', '提示', {
						...
						.then(() => {
							deleteMenu(this.currentMenu.questionId)
								.then(res => {
									this.loading = false
									if (res.code == 0) {
										this.reloadParent()
