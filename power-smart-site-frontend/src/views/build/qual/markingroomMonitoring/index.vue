  ...
  <div class="main">
    ...
    <div class="main-body">
      <el-form v-show="showSearch" ref="queryForm" :model="queryParams" :inline="true" label-width="110px" style="padding-top: 20px;padding-left:10px;">
        ...
        <el-form-item label="监测状态" class="special" prop="alarm" >
          <el-select
            ...
            style="width: 160px">
            <el-option
              v-for="dict in markingroomPoliceOptions"
              :key="dict.dictValue"
      ...
      <el-table v-loading="loading" :height="tableHeight" :data="dataList" @sort-change="sortChange" >
        ...
        <el-table-column label="温度" align="center" prop="temp" sortable="custom" />
        <el-table-column label="湿度" align="center" prop="humidity" sortable="custom" />
        <el-table-column label="监测状态" :formatter="alarmDescribe" align="center" prop="alarm" sortable="custom" />
        <el-table-column label="温度监测状态" :formatter="tempAlarmDescribe" align="center" prop="tempAlarm" sortable="custom" />
        <el-table-column label="湿度监测状态" :formatter="humidityAlarmDescribe" align="center" prop="humidityAlarm" sortable="custom" />
      ...
      <el-dialog :title="title" :visible.sync="open" width="900px" append-to-body>
        <el-form ref="form" :model="form" :rules="rules" label-width="100px" class="add-form">
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="温度" prop="temp" class="add-form-input">
                <el-input-number v-model="form.temp" :precision="2" :step="0.1" ></el-input-number>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="湿度" prop="humidity" class="add-form-input">
                <el-input-number v-model="form.humidity" :precision="2" :step="0.1" ></el-input-number>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="12">
              <el-form-item label="监测状态" prop="alarm" class="add-form-input">
                <el-select
                  v-model="form.alarm"
                  placeholder="请选择"
                  clearable
            ...
            <el-col :span="12">
              <el-form-item label="温度监测状态" prop="tempAlarm" class="add-form-input">
                <el-select
                  v-model="form.tempAlarm"
                  placeholder="请选择"
                  clearable
                  size="small"
                  style="width: 160px">
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="湿度监测状态" prop="humidityAlarm" class="add-form-input">
                <el-select
                  v-model="form.humidityAlarm"
                  placeholder="请选择"
                  clearable
                  size="small"
                  style="width: 160px">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        tempAlarm: [
          { required: true, message: '温度监测状态不能为空', trigger: 'blur' }
        ],
        humidityAlarm: [
          { required: true, message: '湿度监测状态不能为空', trigger: 'blur' }
        ],
        eventTime: [
  ...
  created() {
    this.getDicts('b_qual_markingroom_police').then(response => {
      this.markingroomPoliceOptions = response.data
    })
  ...
  methods: {
    ...
    getList() {
      ...
      request({
        url: 'build/bQualMarkingroomMonitoring/list',
        method: 'post',
    ...
    handleUpdate(row) {
      ...
      request({
        url: 'build/bQualMarkingroomMonitoring/selectById/' + pipeId,
        method: 'get'
      }).then(response => {
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.id != undefined) {
            request({
              url: 'build/bQualMarkingroomMonitoring/edit',
              method: 'post',
              data: this.form,
              headers: {
                'Content-Type': 'application/json;charset=UTF-8'
            ...
            request({
              url: 'build/bQualMarkingroomMonitoring/add',
              method: 'post',
              data: this.form,
              headers: {
                'Content-Type': 'application/json;charset=UTF-8'
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除该数据项?', '警告', {
        ...
        return request({
          url: 'build/bQualMarkingroomMonitoring/deleteDetails/' + id,
          method: 'get'
        })
      }).then(() => {
        this.getList()
    ...
    alarmDescribe(row, column){
      return this.selectDictLabel(this.markingroomPoliceOptions, row.alarm)
    },
    tempAlarmDescribe(row, column){
      return this.selectDictLabel(this.markingroomPoliceOptions, row.tempAlarm)
    },
    humidityAlarmDescribe(row, column){
      return this.selectDictLabel(this.markingroomPoliceOptions, row.humidityAlarm)
    },
