...
<!--        </el-form-item>-->
      ...
      <el-table v-loading="loading" :data="dataList" :height="tableHeight" @sort-change="sortChange">
        ...
        <el-table-column :formatter="qualSpecimenTypeFc" label="试件类型" align="center" prop="specimenType" sortable="custom"/>
        <el-table-column :formatter="formatterTime" label="取样日期" align="center" prop="samplingDate" sortable="custom"/>
          ...
          <el-row>
            ...
            <el-col :span="12">
              <el-form-item label="过磅单号" prop="weighingOrderOn" >
                ...
                <el-button type="primary" round icon="el-icon-search" size="mini" @click="selectWeighing" >查询</el-button>
              </el-form-item>
            </el-col>
            <el-col :span="12">
      ...
      <!-- 新增或修改参数配置对话框 -->
      <el-dialog title="委托见证" :visible.sync="open2" width="900px" append-to-body>
        <el-form ref="formEntrust" :model="formEntrust" :rules="rulesEntrust" label-width="100px" class="add-form">
          <el-row>
            <el-col :span="12">
              <el-form-item label="委托人" prop="client" class="add-form-input">
                <el-input v-model="formEntrust.client" placeholder="请输入" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="委托单位" prop="clientUnit" class="add-form-input">
                <el-input v-model="formEntrust.clientUnit" placeholder="请输入" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="检测单位" prop="testingUnit" class="add-form-input">
                <el-input v-model="formEntrust.testingUnit" placeholder="请输入" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="接受委托人" prop="acceptingClient" class="add-form-input">
                <el-input v-model="formEntrust.acceptingClient" placeholder="请输入" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="委托日期" prop="clientDate" class="add-form-input">
                <el-date-picker
                  v-model="formEntrust.clientDate"
                  type="date"
                  value-format="yyyy-MM-dd HH:mm:ss"
                  placeholder="选择日期"/>
              </el-form-item>
            ...
            <el-col :span="24">
              <el-form-item label="委托见证附件">
                <wk-upload-file
                  ref="wkUploadFile"
                  :auto-upload="false"
                  :batch-id="clientBatchId"
                  name="file"
                  multiple
                  accept="*"
                />
        ...
        <div slot="footer" class="dialog-footer">
          <el-button type="primary" @click="submitFormEntrust">确 定</el-button>
          <el-button @click="cancelEntrust">取 消</el-button>
        </div>
      </el-dialog>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rulesEntrust: {
        ...
        acceptingClient: [
          { required: true, message: '接受委托人不能为空', trigger: 'blur' }
        ],
        clientDate: [
  ...
  methods: {
    ...
    getList() {
      ...
      request({
        url: 'build/bQualSamplingWitness/list',
        method: 'post',
    ...
    handleUpdate(row) {
      ...
      request({
        ...
        if (response.data.specimenDetails != null && response.data.specimenDetails != '') {
          request({
...
/*          if (this.form.specimenDetails == '' || this.form.specimenDetails == undefined) {
    ...
    entrust(row) {
      ...
      request({
        ...
        this.formEntrust = response.data
        this.open2 = true
        if(this.formEntrust.clientBatchId == null || this.formEntrust.clientBatchId == ''){
          this.clientBatchId = guid()
        }else{
          this.clientBatchId = this.formEntrust.clientBatchId
        }
      })

    },
    /** 提交按钮 */
    submitFormEntrust: function() {
      this.$refs['formEntrust'].validate(valid => {
        if (valid) {
          this.formEntrust.clientBatchId = this.clientBatchId
          var fileFlag = this.$refs.wkUploadFile.submitAsync()
          if (!fileFlag) {
            this.$message({
              type: 'error',
          ...
          if (this.formEntrust.id != undefined) {
            request({
              url: 'build/bQualSamplingWitness/edit',
              method: 'post',
              data: this.formEntrust,
              headers: {
                'Content-Type': 'application/json;charset=UTF-8'
              }
            }).then(response => {
