	...
	<div>
		...
		<el-form v-show="true" ref="queryForm" :model="queryParams" :inline="true" label-width="80px"
			...
			<el-form-item label="报警类型">
				<el-select v-model="queryParams.alertTypes" clearable  placeholder="请选择报警类型">
					<el-option
		...
		<div class="table-head-container">
			<el-table v-loading="loading" id="crm-table" :data="list" :height="tableHeight"
				...
				<el-table-column v-for="(item, index) in fieldList" :key="index" :prop="item.prop" :label="item.label"
					:width="item.width" :formatter="formatter" sortable="custom" show-overflow-tooltip align="center">
				</el-table-column>
			</el-table>
	...
	export default {
		...
		methods: {
			...
			getList() {
				...
				this.queryParams.alertType = this.queryParams.alertTypes
				this.queryParams.monitorType = "3"
				selectElevatorAlertPage(this.queryParams)
					.then(res => {
