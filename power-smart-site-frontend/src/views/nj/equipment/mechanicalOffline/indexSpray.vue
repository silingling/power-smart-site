	...
	<div>
		...
		<el-form v-show="true" ref="queryForm" :model="queryParams" :inline="true" label-width="80px"
			...
			<el-form-item label="设备名称">
				<el-input v-model="queryParams.search" placeholder="请输入设备名称或编号" clearable size="small"
...
<script>
  ...
  import {
    assetsList,
  } from '@/views/build/equipment/api/assets'
	...
	export default {
        ...
        assetsList(this.queryParams)
			...
			fieldFormatter(row, column, cellValue) {
				if(column.property == "offlineStatus"){
					switch (cellValue){
						case '1':
							return "上线"
						case '0':
