...
// API
import { noticeAddOrUpateAPI,noticeQueryByIdAPI } from '@/api/oa/notice'
...
export default {
  ...
  created() {
    if (this.action.type == 'update') {
      noticeQueryByIdAPI(this.action.id).then(res => {
        this.formData = {
          title: res.data.title,
          content: res.data.content,
          dep: { staff: res.data.ownerUserList, dep: res.data.deptList }
        }
      })
    }
  },
