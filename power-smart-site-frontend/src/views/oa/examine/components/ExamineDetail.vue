          ...
          <el-table
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <flexbox v-if="item.prop=='hasInvoice'">
                  {{ scope.row.hasInvoice=='0' ?'否' : '是' }}
                </flexbox>
                <flexbox v-else-if="item.prop=='expenseTime'">
                  {{ parseTime(scope.row.expenseTime,'{y}-{m}-{d}')}}
                </flexbox>
                <flexbox v-else>
                  {{ scope.row[item.prop]}}
                </flexbox>
...
export default {
  ...
  data() {
    return {
      ...
      expensesField: [
        { prop: 'description', label: '费用说明' },
        { prop: 'expenseType', label: '费用类型' },
        { prop: 'money', label: '费用金额（元）' },
        { prop: 'expenseTime', label: '费用日期' },
        { prop: 'hasInvoice', label: '有发票' },
        { prop: 'invoiceNum', label: '发票号码' },
