<template>
    <div class="main">
        <div class="main-body">
            <el-form v-show="showSearch" ref="queryForm" :model="queryParams" :inline="true" label-width="80px" style="padding-top: 20px;padding-left:10px;overflow: hidden;">
            ...
            <el-table v-loading="loading" id="crm-table" :span-method="objectSpanMethod" :data="dataList" :height="tableHeight" :cell-class-name="cellClassName" :header-cell-class-name="headerCellClassName" class="n-table--border" use-virtual stripe border highlight-current-row style="width: 100%" @row-click="handleRowClick" @sort-change="sortChange" @selection-change="handleSelectionChange">
                ...
                <el-table-column label="风险辨识点" align="center" prop="riskIdentPoint" show-overflow-tooltip sortable="custom"/>
                <el-table-column label="事故类型" align="center" prop="accidentType" show-overflow-tooltip sortable="custom"/>
                <el-table-column label="风险分级/标识" :formatter="riskLevelIdentFc" align="center" prop="riskLevelIdent" show-overflow-tooltip sortable="custom"/>
                <el-table-column label="防范措施" align="center" prop="preventiveMeasures" show-overflow-tooltip sortable="custom"/>
    ...
    data () {
        return {
            ...
            rules: {
                ...
                riskLevelIdent: [
                    { required: true, message: '风险分级/标识不能为空', trigger: 'change' }
                ],
                preventiveMeasures: [
    ...
    methods: {
        ...
        handleDelete (row) {
            ...
            this.$confirm('是否确认删除该数据项?', '警告', {
                ...
                }).catch(function () { this.msgSuccess('删除失败') })
            }).catch(function () { })
        },
        /** 修改按钮操作 */
        handleUpdate (row) {
