...
<!--      </el-form-item>-->
    ...
    <div class="table-head-container">
      ...
      <el-table v-loading="loading" id="crm-table" :data="list" @sort-change="sortChange" :height="tableHeight" class="n-table--border" use-virtual stripe border highlight-current-row style="width: 100%">
        <el-table-column label="序号" type="index" show-overflow-tooltip width="70"></el-table-column>
        <el-table-column v-for="(item, index) in fieldList" :key="index" :prop="item.prop" :label="item.label" sortable="custom" :width="item.width" :formatter="fieldFormatter"  show-overflow-tooltip>
...
export default {
  ...
  data () {
    return {
      ...
      param: {},
      timeFrame:[
              this.formatterDate(new Date().getTime()-24*60*60*1000*30),this.formatterDate(this.formatterDate(new Date().getTime()+24*60*60*1000))
      ]
    }
  },
  computed: {
