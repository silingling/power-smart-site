...
<script>
  ...
  import {getList, updateStatusData} from "../api/level"
  ...
  export default {
    ...
    methods: {
    	sortChange(column) {
    		if(column.prop=='minRelData'){
    			column.prop= 'min_rel'
    		}
    		if(column.prop=='maxRelData'){
    			column.prop= 'max_rel'
    		}
...
//    },
	  ...
	  // 初始化字典数据
	  initDicts() {
	  	/* this.getDicts('b_eval_score_relation').then(response => {
	  	  this.dictData.relationDict = response.data
	  	}) */
	  },
    }
  }
