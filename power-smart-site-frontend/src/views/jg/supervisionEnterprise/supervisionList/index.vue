      ...
      <el-table
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="200px">
          <template slot-scope="scope">
            ...
            <el-button
			  v-if="scope.row.state == '0' && canHandle('allot')"
              size="mini"
              type="text"
              icon="el-icon-edit"
            ...
            <el-button
			  v-if="scope.row.state == '1' && canHandle('adjustment')"
              size="mini"
              type="text"
              icon="el-icon-edit"
              @click="adjustmentClick(scope.row.id)"
            >调整
            </el-button>
          </template>
        </el-table-column>
...
<script>
  ...
  import {getList, getProjectsList, queryUserList} from '../api/supervisionList'
    ...
    methods: {
      ...
      fieldFormatter(row, column, cellValue) {
		  if (column.property === 'state') {
		  	return this.selectDictLabel(this.dictData.stateDict, row.state)
		  } else if (column.property === 'constructionNature') {
		  	return this.selectDictLabel(this.dictData.constructionNatureOptions, row.constructionNature)
...
//    },
	  ...
	  initDicts() {
		...
		this.getDicts('b_construction_nature').then(response => {
		  this.dictData.constructionNatureOptions = response.data
		})
		// 查询项目数据
