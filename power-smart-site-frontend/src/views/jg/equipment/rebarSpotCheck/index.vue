    ...
    <el-form v-show="true" ref="queryForm" :model="queryParams" :inline="true" label-width="80px"
      ...
      <el-form-item label="项目名称" prop="projectId">
		...
		<el-form-item label="钢筋类型" prop="rebarType">
		  <el-select v-model="queryParams.rebarType" clearable placeholder="请选择钢筋类型">
			<el-option
...
<script>
  ...
  import {getList, deleteDetails, getPeopleList, getProjectsList} from "../api/rebarSpotCheck"
  ...
  export default {
    ...
    methods: {
      ...
      fieldFormatter(row, column, cellValue) {
				if (column.property === 'rebarType') {
					return this.selectDictLabel(this.dictData.rebarTypeDict, row.rebarType)
				} else if (column.property === 'strengthGrade') {
					return this.selectDictLabel(this.dictData.rebarStrengthDict, row.strengthGrade)
				}
...
//    },
	  ...
	  // 初始化字典数据
	  initDicts() {
	  	this.getDicts('b_equipment_rebar_type').then(response => {
	  	  this.dictData.rebarTypeDict = response.data
	  	})
	  	this.getDicts('b_equipment_rebar_strength').then(response => {
	  	  this.dictData.rebarStrengthDict = response.data
	  	})
	  },
	  // 查询项目人员
	  selectPeopleList() {
