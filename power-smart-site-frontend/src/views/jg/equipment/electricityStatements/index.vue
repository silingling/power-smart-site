          ...
          <el-select v-model="queryParams.projectId" placeholder="请选择" clearable size="small" style="width: 160px">
            <el-option v-for="dict in projectOptions" :key="dict.id" :label="dict.name" :value="dict.id" />
				...
				<el-form-item v-if="dateType === '3'" label="时间范围">
					<el-date-picker v-model="dateValue" type="daterange" value-format="yyyy-MM-dd" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期">
					</el-date-picker>
				</el-form-item>
				<el-form-item v-if="dateType === '1'" label="年份">
					<el-date-picker v-model="date" type="year" value-format="yyyy-MM-dd" placeholder="选择年">
					</el-date-picker>
				</el-form-item>
				<el-form-item v-if="dateType === '2'" label="月份">
					<el-date-picker v-model="date" type="month" value-format="yyyy-MM-dd" placeholder="选择月">
					</el-date-picker>
				</el-form-item>
				<el-form-item>
			...
			<div class="flex-box">
				<el-table :data="list" :height="tableHeight" style="width: 100%" highlight-current-row>
					<el-table-column label="序号" fixed type="index" show-overflow-tooltip width="70" align="center">
					</el-table-column>
					<el-table-column fixed prop="projectName" label="项目名称" width="200" align="center" show-overflow-tooltip>
					</el-table-column>
					<el-table-column v-for="(item, index) in fieldList" :key="index" :label="item.label" align="center">
						<el-table-column :key="index" :prop="item.prop" label="用量(kWh)" :width="item.width" :formatter="fieldFormatter" :sortable="false" align="center" show-overflow-tooltip>
						</el-table-column>
					</el-table-column>
				</el-table>
			</div>
...
<script>
	...
	import { getList } from '../api/electricityStatements'
...
//			}
		...
		methods: {
			...
			initData(to) {
				...
				this.list = []
				this.queryParams = {}
				// date 为查询单独时间、dateValue为查询时间范围
				if(to.path.indexOf('/electricityStatementsYear') > 0) {
					this.date = timeToFormatTime(new Date())
					this.dateValue = ''
					this.dateType = '1'
					this.$options.name = 'electricityStatementsYear'
				} else if(to.path.indexOf('/electricityStatementsMonth') > 0) {
					this.date = timeToFormatTime(new Date())
					this.dateValue = ''
					this.dateType = '2'
					this.$options.name = 'electricityStatementsMonth'
				} else if(to.path.indexOf('/electricityStatementsTimeFrame') > 0) {
					this.date = undefined
					...
					this.dateType = '3'
					this.$options.name = 'electricityStatementsTimeFrame'
				}
				this.getTreeList()
				this.getTableList()
			},
			...
			bulkImportClick() {
				...
				this.param = {
					title: "设备用电量导入模板(*)为必填项",
					excelTitle: "设备用电量导入模板",
					colNameCodes: colNameCodes,
					colNames: lists,
					sampleData: dataLists,
					url: 'build/equipmentElectricityRecord/excelImport'
				}
...
@import "../../../../styles/xr-theme.scss";
	...
	.system-nav /deep/ .el-tree--highlight-current .el-tree-node.is-current>.el-tree-node__content {
