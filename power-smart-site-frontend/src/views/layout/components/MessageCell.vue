  ...
  computed: {
    typeObj() {
      ...
      if (this.data.label && this.data.label <= 5) {
        ...
        if ([1, 2, 3].includes(this.data.type)) {
          ...
          key = 'invoice'
        } else if ([90, 91, 92].includes(this.data.type)) {
          key = 'expense-examines'
        }
      }
    ...
    leftContent() {
      return {
        ...
        90: `${this.data.realname}提交了`,
        91: `${this.data.realname}拒绝您的`,
        92: `${this.data.realname}已经审核通过您的`,
      }[this.data.type]
    },
    ...
    rightContent() {
      return {
        ...
        37: `发票审批，请及时处理`,
        90: `费用报销，请及时查看`,
        91: `费用报销审批，拒绝理由：“${this.data.content}”，请及时处理`,
        92: `费用报销，请及时查看`,
      }[this.data.type]
    }
  },
