    ...
    <flexbox
      ...
      <div
        ...
        <flexbox class="task-hd__bottom">
          <flexbox-item class="priority" :span="0.125">
          ...
          <flexbox-item class="label" :span="0.125">
            <flexbox class="head-btn">
            <div class="head-btn__bd"><div class="head-btn__bd--des">里程碑</div></div>
              <el-switch v-model="taskData.isMilestone" :active-value="1" :inactive-value="0" @change="switchChange()"/>
            </flexbox>
          </flexbox-item>
          <flexbox-item>
            <xh-user-cell
...
export default {
  ...
  methods: {
    ...
    switchChange(){
      ...
      setTaskMilestone({
        taskId: this.id,
        status: this.taskData.isMilestone
      })
      .then(res => {
        this.$emit('on-handle', {
          type: 'set-milestone',
          value: this.taskData.isMilestone,
          index: this.detailIndex,
          section: this.detailSection
        })
      })
