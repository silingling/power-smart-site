      ...
      <flexbox v-if="!$slots.icon" class="title"><img   :onerror='`this.src="${require("@/assets/noimgsrc.png")}"`'   
		...
		<el-form v-show="true" ref="queryForm" :model="queryParams" :inline="true" label-width="80px"
			...
			<el-form-item label="描述">
				<el-input v-model="queryParams.describes" placeholder="请输入描述" clearable size="small"
			...
			<el-form-item label="执行器">
				<el-input v-model="queryParams.executor" placeholder="请输入执行器" clearable size="small"
					@keyup.enter.native="getList" />
			</el-form-item>
			<el-form-item label="方法">
				<el-input v-model="queryParams.function" placeholder="请输入方法" clearable size="small"
					@keyup.enter.native="getList" />
			</el-form-item>
			<el-form-item>
			...
			<el-table v-loading="loading" id="crm-table" :data="list" :height="tableHeight"
				...
				<el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="200px"
					...
					<template slot-scope="scope">
						<el-button size="mini" type="text" icon="el-icon-edit" @click="updateClick(scope.row.busintId)">
							修改
						</el-button>
						<el-button size="mini" type="text" icon="el-icon-delete"
							@click="deleteClick(scope.row.busintId)">删除
						</el-button>
						<el-button size="mini" type="text" icon="el-icon-delete" @click="ceshi(scope.row.busintId)">测试
						</el-button>
					</template>
				</el-table-column>
			</el-table>
		...
		<el-dialog
		  ...
		  :before-close="handleClose">
			<el-input type="textarea" :rows="4" v-model="ceshiData" ></el-input>
		</el-dialog>
	</div>
</template>
		...
		methods: {
			...
			fieldFormatter(row, column, cellValue) {
				if (column.property == 'type') {
					return this.typeDict[row[column.property]];
				}
			...
			ceshi(id) {
				this.$prompt('JSON格式参数,如：{"projectId":51,"tenantId":15190}', '提示', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
				}).then(({
					value
