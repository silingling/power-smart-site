      ...
      <flexbox v-if="!$slots.icon" class="title"><img   :onerror='`this.src="${require("@/assets/noimgsrc.png")}"`'
		...
		<el-form v-show="true" ref="queryForm" :model="queryParams" :inline="true" label-width="80px"
			...
			<el-form-item label="任务">
				<el-select v-model="queryParams.jobCode" clearable placeholder="请选择" filterable >
			...
			<el-form-item label="项目">
				<el-select v-model="queryParams.projectId" clearable placeholder="请选择" filterable >
					<el-option v-for="item in projectNames" :key="item.id" :label="item.projectName" :value="item.id">
					</el-option>
				</el-select>
			</el-form-item>
			<el-form-item label="业务接口">
				<el-select v-model="queryParams.busintId" clearable placeholder="请选择" filterable >
					<el-option v-for="item in busints" :key="item.busintId" :label="item.name" :value="item.busintId">
					</el-option>
				</el-select>
			</el-form-item>
			<el-form-item label="对接平台">
				<el-select v-model="queryParams.platformId" clearable placeholder="请选择" filterable >
		...
		<div v-empty="!canHandle('list')" xs-empty-icon="nopermission" xs-empty-text="暂无权限" class="crm-container">
			<el-table v-loading="loading" id="crm-table" :data="list" :height="tableHeight"
				...
				<el-table-column v-for="(item, index) in fieldList" :key="index" :fixed="index==0" :prop="item.prop"
					:label="item.label" :width="item.width" :formatter="fieldFormatter" :sortable="false"
				...
				<el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="200px"
					...
					<template slot-scope="scope">
						<el-button size="mini" type="text" icon="el-icon-edit"
							@click="updateClick(scope.row.jobbusintId)">
							修改
						</el-button>
						<el-button size="mini" type="text" icon="el-icon-delete"
							@click="deleteClick(scope.row.jobbusintId)">删除
						</el-button>
		...
		<modify v-if="modifyShow" :businessType="businessType" :action="action" @close="modifyShow = false"
			@save-success="getList" />
	</div>
</template>
	...
	export default {
		...
		methods: {
			...
			fieldFormatter(row, column, cellValue) {
				var str = row[column.property] === '' || row[column.property] === null ? '--' : row[column.property]
				if (column.property == 'jobName') {
					this.jobNames.forEach(function(val) {
						if (row.jobCode == val.id) {
							str = val.jobDesc
							return;
						}
					})
				}
				if (column.property == 'jobApi') {
					this.jobNames.forEach(function(val) {
						if (row.jobCode == val.id) {
							str = val.executorHandler
							return;
						}
					})
				}
			...
			ceshi(id) {
				this.$prompt('JSON格式参数', '提示', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					inputPlaceholder: '填写JSON字符串如：{"projectId":51,"tenantId":15190}'
				}).then(({
					value
				}) => {
					...
					getTest(data).then(res => {
						if (res.code == 0) {
							...
							this.ceshiData = JSON.stringify(res.data)
						} else {
							this.$message.error(res.msg);
						}
						this.loading = false;
