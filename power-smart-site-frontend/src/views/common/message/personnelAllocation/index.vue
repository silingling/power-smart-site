<template>
  <div>
    <!--<div class="c-container">
            <flexbox v-if="!$slots.icon" class="title"><img   :onerror='`this.src="${require("@/assets/noimgsrc.png")}"`'    :src="titleIcon" class="title-icon">消息管理
            </flexbox>
        </div>-->
    <!-- 头部：搜索、操作 start -->
    <el-form v-show="true" ref="queryForm" :model="queryParams" :inline="true" label-width="80px"
      ...
      <el-form-item label="描述">
        <el-input v-model="queryParams.descr" placeholder="请输入描述" clearable size="small"
                  @keyup.enter.native="getList"/>
      </el-form-item>
      <el-form-item label="标识">
        <el-input v-model="queryParams.code" placeholder="请输入标识" clearable size="small"
                  @keyup.enter.native="getList"/>
      </el-form-item>
      <el-form-item>
    ...
    <div v-empty="!canHandle('list')" xs-empty-icon="nopermission" xs-empty-text="暂无权限" class="crm-container">
      <el-table v-loading="loading" id="crm-table" :data="list" :height="tableHeight"
        ...
        <el-table-column show-overflow-tooltip type="index" align="center" width="55" fixed="left"/>
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="260px"
          ...
          <template slot-scope="scope">
            <el-button size="mini" type="text" icon="el-icon-edit" @click="updateClick(scope.row.code)">配置
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 新建 -->
    <create v-if="createShow" :businessType="businessType" :code="code" @close="createShow = false"
  ...
  export default {
    ...
    methods: {
      ...
      fieldFormatter(row, column, cellValue) {
        ...
        } else if (column.property == 'ifSendMq' ||
          column.property == 'ifSendWeb' ||
          column.property == 'isSys') {
          return this.yesOrNoDict[row[column.property]];
        } else if (column.property == 'deptIds' && row.deptList.length > 0) {
          var str = ''
          for (var i = 0; i < row.deptList.length; i++) {
            if (i > 0) {
              str += ','
            }
            const item = row.deptList[i]
            str += item.name
          }
          return str
        } else if (column.property == 'ownerUserIds' && row.ownerUserList.length > 0) {
          var str = ''
          for (var i = 0; i < row.ownerUserList.length; i++) {
            if (i > 0) {
              str += ','
            }
            const item = row.ownerUserList[i]
            str += item.realname
          }
          return str
        }
