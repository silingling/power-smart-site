      ...
      @search="headerSearch" />-->
		<div class="role-box">
			...
			<div v-loading="roleMenuLoading" class="nav">
				<div class="nav__hd">
					{{ title }}
					<el-button type="text" icon="el-icon-circle-plus" class="add-btn" @click="newRoleBtn">创建角色
					</el-button>
				</div>
				<!-- 角色列表 -->
				<div class="role-nav-box">
					<div v-for="(item, index) in roleList" :key="index"
						:class="{'is-select' : item.roleId == roleActive.roleId}" class="menu-item"
						@click="roleMenuSelect(item)">
						{{ item.roleName }}
						<div v-if="item.remark != 'admin' && item.remark != 'project'" class="icon-close">
							<el-dropdown trigger="click" @command="roleHandleClick">
								<i class="el-icon-arrow-down" @click="roleDropdownClick(item)" />
								<el-dropdown-menu slot="dropdown">
									<el-dropdown-item command="copy">复制</el-dropdown-item>
									<el-dropdown-item command="edit">编辑</el-dropdown-item>
									<el-dropdown-item command="delete" :disabled="item.roleId == 180284">删除
									</el-dropdown-item>
								</el-dropdown-menu>
							</el-dropdown>
						</div>
			...
			<el-dialog :title="roleTitle" :visible.sync="newRoleVisible" :before-close="newRoleClose"
				...
				<el-input v-model="role.title" :maxlength="100" class="input-role" />
				<label class="label-title">角色类型</label>
				<el-select :maxlength="100" class="input-role" v-model="role.roleType" placeholder="请选择" clearable
					size="small" style="width: 160px">
					<el-option v-for="dict in roleTypeList" :key="dict.dictValue" :label="dict.dictLabel"
				...
				<el-tabs v-model="mainMenuIndex">
					<el-tab-pane label="角色员工" name="user">
						<div v-loading="userLoading" class="content-table">
							<flexbox class="content-table-header">
								...
								<el-button size="medium" class="xr-btn--orange" type="primary" @click="addEmployees">
									关联员工 </el-button>
							</flexbox>
							<el-table :data="tableData" :height="tableHeight" style="width: 100%">
								<el-table-column label="序号" type="index" show-overflow-tooltip width="70" fixed />
								<el-table-column v-for="(item, index) in tableList" :prop="item.field"
									:label="item.label" :key="index" show-overflow-tooltip />
                      ...
                      @click="editBtn(scope.row)"></span> -->
							...
							<div class="p-contianer">
								<el-pagination :current-page="currentPage" :page-sizes="pageSizes"
									:page-size.sync="pageSize" :total="total" class="p-bar" background
					...
					<el-tab-pane v-if="roleActive && showRuleSet" label="角色权限" name="rule">
						...
						<div v-loading="ruleLoading" class="jurisdiction-box">
							<el-button v-if="roleActive" size="medium" type="primary" class="jurisdiction-edit"
								@click="ruleSubmit"> 保存 </el-button>
							<el-tabs v-model="ruleMenuIndex">
								<el-tab-pane v-for="(item, index) in ruleMenuList" :key="index" :label="item.label"
									:name="item.index">
									<div v-if="item.type == 'tree'" :style="{ height: treeHeight + 'px'}"
										class="jurisdiction-content">
										<div class="jurisdiction-content-checkbox">
											<el-tree :data="item.data" :ref="'tree' + item.index" :indent="0"
												:expand-on-click-node="false" :props="defaultProps" show-checkbox
												node-key="menuId" style="height: 0;" empty-text="" default-expand-all>
												<span slot-scope="{ node }"
													:class="{ 'node-label': node.level == 1 || node.level == 2} ">{{ node.label }}
													<el-button v-if="node.level == 2 && canSetField(node.data.realm)"
														icon="wk wk-manage" type="text" @click="fieldSetClick(node)">
														字段授权</el-button>
												</span>
											</el-tree>
										</div>
					...
					<el-tab-pane v-if="roleActive && showRuleSet" label="角色仪表盘权限" name="instru">
						<instrument ref="instrument" :role-id="roleActive.roleId"></instrument>
					</el-tab-pane>
					<el-tab-pane v-if="roleActive && showRuleSet" label="大屏权限" name="screen">
						<div>
							
							<el-button size="medium" type="primary" class="jurisdiction-edit"
								@click="dpSubmit"> 保存 </el-button>
								<el-tabs v-model="dpShow">
									<el-tab-pane class="dpqhy" name="jgd" label="监管端">
										<el-checkbox class="dpqc_box" v-model="dpForm.supervise.isOverview">平台总览</el-checkbox>
										<el-checkbox class="dpqc_box" v-model="dpForm.supervise.isProAnalysis">项目分析</el-checkbox>
										<el-checkbox class="dpqc_box" v-model="dpForm.supervise.isPolAnalysis">报警分析</el-checkbox>
										<el-checkbox class="dpqc_box" v-model="dpForm.supervise.isVideo">实时监控</el-checkbox>
										<el-checkbox class="dpqc_box" v-model="dpForm.supervise.isEnvironment">实时环境</el-checkbox>
										<el-checkbox class="dpqc_box" v-model="dpForm.supervise.isIntelligence">AI智能分析</el-checkbox>
										<el-checkbox class="dpqc_box" v-model="dpForm.supervise.isTower">塔机设备</el-checkbox>
										<el-checkbox class="dpqc_box" v-model="dpForm.supervise.isElevator">升降机设备</el-checkbox>
									</el-tab-pane>
									<el-tab-pane name="gdd" label="工地端">
										<el-checkbox class="dpqc_box" v-model="dpForm.construction.isIndex">首页</el-checkbox>
										<el-checkbox class="dpqc_box" v-model="dpForm.construction.isVideo">监控</el-checkbox>
										<el-checkbox class="dpqc_box" v-model="dpForm.construction.isEquipment">机械设备</el-checkbox>
										<el-checkbox class="dpqc_box" v-model="dpForm.construction.isEnvironment">环境监测</el-checkbox>
										<el-checkbox class="dpqc_box" v-model="dpForm.construction.isSecurity">安全管理</el-checkbox>
										<el-checkbox class="dpqc_box" v-model="dpForm.construction.isQuality">质量管理</el-checkbox>
										<el-checkbox class="dpqc_box" v-model="dpForm.construction.isLabou">劳务分析</el-checkbox>
										<el-checkbox class="dpqc_box" v-model="dpForm.construction.isSpeed">进度管理</el-checkbox>
										<el-checkbox class="dpqc_box" v-model="dpForm.construction.isPolice">报警统计分析</el-checkbox>
									</el-tab-pane>
									<el-tab-pane name="swdp" label="三维大屏">
										<el-checkbox class="dpqc_box" v-model="dpForm.threeDimensional.isOverview">概况</el-checkbox>
										<el-checkbox class="dpqc_box" v-model="dpForm.threeDimensional.isPolice">报警</el-checkbox>
										<el-checkbox class="dpqc_box" v-model="dpForm.threeDimensional.isVideo">监控</el-checkbox>
										<el-checkbox class="dpqc_box" v-model="dpForm.threeDimensional.isEnvironment">环境</el-checkbox>
										<el-checkbox class="dpqc_box" v-model="dpForm.threeDimensional.isTower">塔机</el-checkbox>
										<el-checkbox class="dpqc_box" v-model="dpForm.threeDimensional.isElevator">升降机</el-checkbox>
										<el-checkbox class="dpqc_box" v-model="dpForm.threeDimensional.isFouPit">基坑</el-checkbox>
										<el-checkbox class="dpqc_box" v-model="dpForm.threeDimensional.isHighFormwork">高支模</el-checkbox>
										<el-checkbox class="dpqc_box" v-model="dpForm.threeDimensional.isHidden">隐患</el-checkbox>
										<el-checkbox class="dpqc_box" v-model="dpForm.threeDimensional.isPeople">人员</el-checkbox>
										<el-checkbox class="dpqc_box" v-model="dpForm.threeDimensional.isSpeed">进度</el-checkbox>
									</el-tab-pane>
								</el-tabs>
		...
		<!-- 关联员工 -->
		<relate-empoyee :visible.sync="relateEmpoyeeShow" :role-id="roleId" @save="employeesSave" />
		<!-- 字段授权 -->
	...
	export default {
		...
		data() {
			return {
				...
				//大屏权限
				dpShow: 'jgd',
				dpForm: {supervise:{},construction:{},threeDimensional:{}},
			}
		},
		...
		watch: {},
		created() {
			this.getDicts('b_role_type').then(response => {
		...
		methods: {
			 ...
			 */
			getRulesList() {
				systemRuleByTenantAPI().then(res => {
					if (res.data.all) {
			...
			roleMenuSelect(val) {
				...
				this.refreshUserList()
				this.getMenuDp()
				this.$refs.instrument.getModelSort(this.roleActive.roleId)
			},
				...
				if (this.roleActive && this.ruleMenuList.length) {
					...
					for (let index = 0; index < this.ruleMenuList.length; index++) {
						...
						if (element.type == 'tree') {
							//console.log("=== this.roleActive.rules[element.index]"+JSON.stringify( this.roleActive.rules[element.index]))
							//console.log("=== element.data[0]"+JSON.stringify( element.data[0]))
							// console.log("=== this.getRoleRules"+JSON.stringify( this.getRoleRules(
...
//				this.ruleLoading = true
				this.$set(this.dpForm,'supervise',{})
				this.$set(this.dpForm,'construction',{})
				this.$set(this.dpForm,'threeDimensional',{})
				
//				this.dpForm={supervise:{},construction:{},threeDimensional:{}};
				getMenuDbByRoleId(this.roleActive.roleId)
					.then(res => {
						var than = this;
						
						res.data.forEach(function(iem){
							if(iem.platform == "supervise"){
//								than.dpForm.supervise[iem.menu] = true
								than.$set(than.dpForm.supervise,iem.menu,true)
							}
							if(iem.platform == "construction"){
//								than.dpForm.construction[iem.menu] = true
								than.$set(than.dpForm.construction,iem.menu,true)
							}
							if(iem.platform == "threeDimensional"){
//								than.dpForm.threeDimensional[iem.menu] = true
								than.$set(than.dpForm.threeDimensional,iem.menu,true)
							}
						})
			...
			canSetField(type) {
				...
				return ['leads', 'customer', 'contacts', 'business', 'contract', 'receivables', 'product', 'visit',
						'expense'
					].includes(type) &&
					(this.ruleMenuIndex === 'crm' || this.ruleMenuIndex === 'bi')
			},
