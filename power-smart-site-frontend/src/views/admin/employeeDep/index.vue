<template>
  <div class="employee-dep-management">
    ...
    <div class="system-content" style="margin-top: 10px">
      <!-- 左边导航栏 -->
      <div
      ...
      <div class="system-view-table flex-index">
        <flexbox
          ...
          <div>
            <el-button
              v-if="strucSaveAuth && userInfo.deptId==1"
              type="text"
              icon="el-icon-circle-plus"
              @click="addAdmin">添加租户管理员
            </el-button>
            <el-button
              v-if="userSaveAuth"
              type="text"
              icon="el-icon-circle-plus"
              @click="addEmployee">添加员工
            </el-button>
            <el-button
              v-if="strucSaveAuth && currentMenuData && currentMenuData.id"
              type="text"
              icon="el-icon-circle-plus"
              @click="appendStruc(currentMenuData)">创建子部门
            </el-button>
            <el-dropdown
        ...
        <flexbox
          ...
          <flexbox class="selection-items-box">
            <el-button
              ...
              :key="index"
              type="primary"
              @click.native="selectionBarClick(item.type)">{{ item.name }}
            </el-button>
          </flexbox>
        </flexbox>
        <div class="flex-box">
    ...
    <el-dialog class="updataopen" :title="grantTitle" :visible.sync="grantOpen" width="1200px" append-to-body>
      <GantProject :userId="grantUserId"/>
    ...
    <el-dialog
      ...
      <div class="el-password">
        ...
        <div
          class="tips"
          style="margin-top: 20px;">重置登录帐号后，员工需用新账号登录。请及时告知员工，确保正常使用
        </div>
      </div>
      <span
        slot="footer"
...
//给人员添加项目授权组件
import GantProject from './components/grant_project'
...
export default {
  ...
  data() {
    return {
      ...
      fieldList: [
        ...
        {field: 'deptName', value: '部门', type: 'select', width: '150'},
        {field: 'post', value: '岗位', width: '120'},
        {
          field: 'parentName',
          value: '直属上级',
          type: 'select',
  ...
  computed: {
    ...
    tableList: function () {
      ...
      } else if (this.dialogTitle === '新建租户管理员') {
        return [
          {field: 'username', value: '手机号（登录名）'},
          {field: 'password', value: '登录密码'},
          {field: 'realname', value: '姓名'},
  ...
  methods: {
    ...
    grantUser(row) {
      console.log(row)
      this.grantUserId = row.userId;
      this.grantTitle = row.realname + "-授权项目";
      this.grantOpen = true;
    },
    ...
    addStruc() {
      const id =
        this.allDepData && this.allDepData.length ? this.allDepData[0].deptId : 1
      if (id) {
    ...
    newDialogSubmit() {
      this.$refs.dialogRef.validate(valid => {
        if (valid) {
          if (this.dialogTitle == '新建员工' || this.dialogTitle == '新建租户管理员') {
            this.loading = true
