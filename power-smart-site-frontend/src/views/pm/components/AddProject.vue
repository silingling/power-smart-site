    ...
    <flexbox
      ...
      <div class="content">
        <div class="project-name">
          <div class="label color-label" style="width: 100px;clear: left;float: left;line-height: 3">项目名称</div>
          ...
          <div class="label color-label" style="width: 100px;clear: left;float: left;line-height: 3">关联合同</div>
          <div style="float: left;width: 40%;">
            <CrmRelativeCell
                    :value="contractIdItem.value"
                    :item="contractIdItem"
                    :relation="contractIdItem.relation"
                    :relative-type="contractIdItem.formType"
                    @value-change="contractValueChange">
          </CrmRelativeCell></div>
          <br/>
          <div class="label color-label" style="width: 100px;clear: left;float: left;line-height: 3">关联商机</div>
          <div style="float: left;width: 40%;">
            <CrmRelativeCell
                    :value="contractIdItem.value"
                    :item="businessIdItem"
                    :relative-type="businessIdItem.formType"
                    @value-change="businessValueChange">
            </CrmRelativeCell></div>
          <div class="color-box" style="clear: left">
            <span
        ...
        <div class="cover">
          ...
          <flexbox
            ...
            <flexbox-item
              ...
              <img   :onerror='`this.src="${require("@/assets/noimgsrc.png")}"`'    v-src="item.url" class="cover-img">
...
export default {
  ...
  data() {
    return {
      ...
      contractId: null,
      businessId: null,
      detail: {customerId:0,customerName:""},
      businessIdItem:{
        name: '商机编号',
        field: 'businessId',
        formType: 'business',
  ...
  methods: {
    ...
    contractValueChange(data) {
      const dataValue = data.value
      this.contractId = dataValue && dataValue.length ? dataValue[0].contractId : null
      this.detail.customerId = dataValue && dataValue.length ? dataValue[0].customerId : null
      this.detail.customerName = dataValue && dataValue.length ? dataValue[0].customerName : null
    },
    businessValueChange(data) {
      const dataValue = data.value
      this.businessId = dataValue && dataValue.length ? dataValue[0].businessId : null
      this.detail.customerId = dataValue && dataValue.length ? dataValue[0].customerId : null
      this.detail.customerName = dataValue && dataValue.length ? dataValue[0].customerName : null
    },
    /**
    ...
    submitBtn() {
      ...
      // console.log("===contractId="+JSON.stringify(this.contractId))
      // console.log("===businessId="+JSON.stringify(this.businessId))
      if (this.contractId==null && this.businessId==null) {
        this.$message.error('请关联商机或合同')
        return
      }
      this.loading = true
      const params = {
