  ...
  <transition name="slide-fade">
    <el-card
      ...
      <div class="content">
        ...
        <flexbox class="menu-list">
          <div class="item-label" style="width: 80%;">里程碑</div>
          <el-switch v-model="isMilestone" :active-value="1" :inactive-value="0" style="margin-top: 2px" @change="switchChange()"/>
        </flexbox>
        <div
...
export default {
  ...
  created() {
    ...
    this.isMilestone = this.data.isMilestone
    this.getUserList()
    this.getTagList()
  ...
  methods: {
    ...
    switchChange(){
      this.$emit('change', this.data.userIds, this.data.timeId, this.data.tagIds, this.search, this.isMilestone)
    },
    rowChecked(val) {
      if (val.type == 'time') {
      ...
      this.$emit('change', userIds, timeId, tagIds, this.search, this.isMilestone)
    },
    ...
    resetBtn() {
      ...
      this.search = ''
      this.isMilestone = 0
      this.$emit('change', [], '', [], this.search, this.isMilestone)
    },
