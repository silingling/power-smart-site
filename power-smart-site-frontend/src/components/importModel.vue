    ...
    <div class="dialog-body">
      ...
      <div
        ...
        <div
          class="download"
          @click="download">
          点击下载《{{param.excelTitle}}》</div>
        <div class="content content-tips">
          <div>操作步骤：</div>
          <div>1、下载《{{param.excelTitle}}》</div>
          <div>2、打开《{{param.excelTitle}}》将对应字段信息输入或粘贴进本表。为保障粘贴信息被有效导入，请使用纯文本或数字</div>
          <div>3、信息输入完毕，点击“选择文件”按钮，选择excel文件上传</div>
          <div>4、点击“确定”开始进行数据导入</div>
        </div>
      </div>
      ...
      <div
        ...
        <div class="result-info">
          ...
          <p class="result-info__des">{{conent}}</p>
...
import { downloadFileAPI ,excelImpModelExport ,excelImportAPI} from '@/api/common'
  ...
  methods: {
    ...
    updateFile(result) {
      if (!this.file.name) {
        ...
        this.loading = true
        if(this.param.excelTitle == "wifi教育题库导入模板"){
          excelImportAPI({ file: this.file,questTypeId:this.param.questTypeId },this.param.url) .then(res => {
            this.loading = false
            if (result) {
              result(res)
            }
          ...
          excelImportAPI({ file: this.file },this.param.url) .then(res => {
            this.loading = false
            if (result) {
              result(res)
            }
     ...
     */
    download() {
  /*    downloadFileAPI('/adminFile/down/123').then(res => {
        const blob = new Blob([res.data], {
          type: ''
        })
        downloadFileWithBuffer(blob, 'attendance_import.xlsx')
      }).catch(() => {
      })*/

      //console.log(this.param)
      excelImpModelExport(this.param).then(res => {
      ...
      //window.open("http://www.oceanus-health.com/hdwxsc/wx/jump/download2?id=06d79171ab944a3881233f811b79e565")
    },
